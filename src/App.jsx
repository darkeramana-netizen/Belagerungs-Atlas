...
// ─── WORLD MAP POSITIONS ──────────────────────────────────────────────────
const COORDS = {
  krak:{x:62,y:39},masada:{x:61,y:41},carcassonne:{x:47,y:29},chateau_gaillard:{x:46,y:26},
  harlech:{x:44,y:22},akkon:{x:61,y:40},himeji:{x:83,y:33},alamut:{x:64,y:35},
  constantinople:{x:57,y:33},coucy:{x:47,y:26},babylon:{x:63,y:38},mont_michel:{x:45,y:26},
  alhambra:{x:44,y:34},mehrangarh:{x:68,y:39},caernarvon:{x:44,y:22},windsor:{x:45,y:24},
  bodiam:{x:45,y:24},rhodes:{x:57,y:35},angkor:{x:79,y:47},persepolis:{x:65,y:38},
  knossos:{x:56,y:35},troy:{x:56,y:33},great_wall:{x:78,y:31},morville:{x:46,y:26},
  // Fantasy in separate quadrant top-left — MITTELERDE oben, WESTEROS unten
  helmsdeep:{x:8,y:10},minas_tirith:{x:12,y:12},gondolin:{x:6,y:7},barad_dur:{x:14,y:14},
  erebor:{x:16,y:8},isengard:{x:9,y:11},minas_morgul:{x:13,y:13},angband:{x:5,y:5},
  khazad_dum:{x:10,y:9},edoras:{x:11,y:12},
  // Game of Thrones — WESTEROS unten im Fantasy-Bereich
  winterfell:{x:7,y:22},kings_landing:{x:13,y:25},harrenhal:{x:11,y:23},
};
  ...
  return(
    <div style={{marginBottom:"8px"}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:"3px"}}>
        <span style={{fontSize:"9px",color:"#5a4a30",letterSpacing:"1px"}}>{label.toUpperCase()}</span>
        <span style={{fontSize:"10px",fontWeight:"bold",color:rCol(value),fontFamily:"monospace"}}>{value}</span>
      </div>
...
function RadarChart({castle,compare}){
  ...
  return(
    <svg viewBox="0 0 100 100" style={{width:"100%",maxWidth:"200px",display:"block",margin:"0 auto"}}>
      ...
      {cats.map((c,i)=>{const p=axPt(i,1.22);return <text key={i} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle" fill={`${ac}66`} fontSize="5" fontFamily="serif">{c.l}</text>;}) }
...
const CASTLE_PLANS = {
  ...
  krak: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="152" y="75" fill={`${ac}44`} fontSize="5" fontFamily="serif">Zwinger</text>
      {/* Inner wall ring */}
      ...
      <text x="110" y="84" textAnchor="middle" fill={`${ac}77`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("in")}>INNERER RING</text>
      {/* Donjon */}
      ...
      <text x="110" y="112" textAnchor="middle" fill="#e8c860" fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("kp")}>DONJON</text>
      {/* Cisterns */}
      ...
      <text x="148" y="137" textAnchor="middle" fill="#4488cc" fontSize="5.5" style={{cursor:"pointer"}} onClick={()=>onZone("ci")}>Zistern</text>
      {/* North weakness */}
      ...
      <text x="110" y="30" textAnchor="middle" fill="#cc4444" fontSize="6" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("nw")}>⚠ NORD-FLANKE</text>
      {/* Gates */}
      <rect x="134" y="141" width="10" height="7" rx="1" fill="#c9a84c" opacity="0.8"/>
      <text x="146" y="158" fill={`${ac}66`} fontSize="5" fontFamily="serif">Haupttor</text>
      {/* Compass */}
      ...
      {/* Title */}
      <text x="110" y="190" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">KRAK DES CHEVALIERS</text>
    </g>
  ),
  ...
  masada: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="118" y="50" textAnchor="middle" fill={`${ac}44`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("cl")}>STEILKLIPPEN (400m)</text>
      {/* Casemate wall */}
      ...
      <text x="106" y="45" textAnchor="middle" fill={`${ac}88`} fontSize="5.5" fontFamily="serif">Nordpalast</text>
      {/* Southern Palace */}
      <rect x="78" y="108" width="55" height="30" rx="2" fill="rgba(201,168,76,0.09)" stroke={`${ac}44`} strokeWidth="0.8"/>
      <text x="105" y="126" textAnchor="middle" fill={`${ac}66`} fontSize="5.5" fontFamily="serif">Südpalast</text>
      {/* Cisterns */}
      ...
      <text x="158" y="89" textAnchor="middle" fill="#4488cc" fontSize="5" style={{cursor:"pointer"}} onClick={()=>onZone("cs")}>Zistern</text>
      ...
      <text x="19" y="90" textAnchor="middle" fill="#cc4444" fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("ws")}>⚠ WEST-</text>
      <text x="19" y="97" textAnchor="middle" fill="#cc4444" fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("ws")}>SPORN</text>
      {/* Roman ramp arrow */}
      ...
      <text x="15" y="112" textAnchor="middle" fill="rgba(204,68,68,0.5)" fontSize="4.5">RAMPE</text>
      ...
      <text x="110" y="182" textAnchor="middle" fill="rgba(180,50,20,0.35)" fontSize="4.5">Römische Circumvallation</text>
      {/* Compass + title */}
      ...
      <text x="110" y="196" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">MASADA — TAFELBERG 73 n.Chr.</text>
    </g>
  ),
  ...
  carcassonne: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="170" y="58" fill={`${ac}44`} fontSize="5" fontFamily="serif">Äußere</text>
      <text x="170" y="65" fill={`${ac}44`} fontSize="5" fontFamily="serif">Vormauer</text>
      {/* Zwinger space */}
      <ellipse cx="110" cy="105" rx="68" ry="60" fill="rgba(45,36,14,0.12)" stroke="rgba(90,70,25,0.12)" strokeWidth="0.5"/>
      <text x="155" y="100" fill="rgba(100,80,30,0.4)" fontSize="4.5" fontFamily="serif">Zwinger</text>
      {/* Inner main wall */}
      ...
      <text x="99" y="97" textAnchor="middle" fill="#e8c860" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ch")}>Comtal</text>
      {/* Water vulnerability */}
      ...
      <text x="162" y="143" textAnchor="middle" fill="#cc4444" fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("wt")}>⚠ Wasser</text>
      {/* Narbonne gate */}
      <rect x="100" y="178" width="20" height="9" rx="1" fill="#c9a84c" opacity="0.7"/>
      <text x="110" y="193" textAnchor="middle" fill={`${ac}66`} fontSize="5" fontFamily="serif">Narbonne-Tor</text>
      {/* Compass + title */}
      ...
      <text x="110" y="200" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">CARCASSONNE</text>
    </g>
  ),
  ...
  helmsdeep: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="12" textAnchor="middle" fill="rgba(204,68,68,0.45)" fontSize="5.5">← URUK-HAI ANGRIFF →</text>
      {/* Deeping Wall */}
      ...
      <text x="110" y="116" textAnchor="middle" fill={`${ac}ee`} fontSize="6.5" fontFamily="serif" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("dw")}>HORNMAUER</text>
      {/* Drain — FATAL */}
      ...
      <text x="90" y="130" textAnchor="middle" fill="#ff4444" fontSize="6" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("dr")}>⚠⚠</text>
      <text x="90" y="138" textAnchor="middle" fill="#ff4444" fontSize="5" style={{cursor:"pointer"}} onClick={()=>onZone("dr")}>DRAIN</text>
      <text x="90" y="146" textAnchor="middle" fill="rgba(255,68,68,0.7)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("dr")}>(Sprengstoff!)</text>
      {/* Hornburg */}
      ...
      <text x="152" y="90" textAnchor="middle" fill="#aa8844" fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("hb")}>HORN-</text>
      <text x="152" y="98" textAnchor="middle" fill="#aa8844" fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("hb")}>BURG</text>
      ...
      <text x="110" y="162" textAnchor="middle" fill="#7a6a50" fontSize="6.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ag")}>AGLAROND</text>
      <text x="110" y="172" textAnchor="middle" fill="#7a6a50" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ag")}>HÖHLEN</text>
      {/* Compass */}
  ...
  minas_tirith: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="10" textAnchor="middle" fill="rgba(204,68,68,0.38)" fontSize="5.5">← SAURONS HEER — ANGRIFF →</text>
      {/* Pelennor marker */}
      ...
      <text x="35" y="51" textAnchor="middle" fill="#774433" fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("pf")}>⚠ Pelennor</text>
      ...
      {[
        ...
        <g key={id}>
          ...
          <text x="110" y={cy+5} textAnchor="middle" fill={c} fontSize="6"
      ...
      <text x="110" y="196" textAnchor="middle" fill="#cc4444" fontSize="5.5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("gt")}>⚠ HAUPTTOR (Grond!)</text>
      {/* White tree */}
      <circle cx="110" cy="115" r="4" fill="rgba(255,255,255,0.1)" stroke="rgba(255,255,255,0.2)" strokeWidth="0.8"/>
      <text x="110" y="117" textAnchor="middle" fill="rgba(255,255,255,0.2)" fontSize="4">🌳</text>
      {/* Compass */}
      <text x="200" y="25" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="200" y1="28" x2="200" y2="44" stroke={`${ac}44`} strokeWidth="1"/>
    </g>
  ...
  constantinople: ({ac,sel,onZone}) => (
    <g>
      ...
      {/* Sea labels */}
      <text x="87" y="162" textAnchor="middle" fill="rgba(68,136,204,0.45)" fontSize="6" fontFamily="serif">Marmarameer</text>
      <text x="197" y="75" textAnchor="middle" fill="rgba(68,136,204,0.4)" fontSize="5.5" fontFamily="serif" transform="rotate(90 197 75)">Bosporus</text>
      {/* City land */}
      ...
      <text x="87" y="11" textAnchor="middle" fill="#5599cc" fontSize="5.5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("gr")}>VORGRABEN — 18m BREIT</text>
      {/* Outer wall */}
      ...
      <text x="87" y="25" textAnchor="middle" fill={`${ac}88`} fontSize="5.5" style={{cursor:"pointer"}} onClick={()=>onZone("w1")}>ÄUSSERE MAUER</text>
      ...
      <text x="87" y="40" textAnchor="middle" fill={`${ac}ee`} fontSize="5.5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("w2")}>INNERE MAUER — 12m HOCH, 5m DICK</text>
      {/* City interior */}
      ...
      <text x="87" y="95" textAnchor="middle" fill="rgba(201,168,76,0.2)" fontSize="5.5" style={{cursor:"pointer"}} onClick={()=>onZone("ct")}>500.000 Einwohner · 1000 Jahre Hauptstadt</text>
      {/* Hagia Sophia dome */}
      <ellipse cx="95" cy="108" rx="10" ry="8" fill="rgba(201,168,76,0.16)" stroke={`${ac}44`} strokeWidth="0.8"/>
      <text x="95" y="111" textAnchor="middle" fill={`${ac}55`} fontSize="4">Hagia Sophia</text>
      {/* Hippodrome */}
      <ellipse cx="130" cy="112" rx="15" ry="8" fill="rgba(100,80,30,0.12)" stroke={`${ac}33`} strokeWidth="0.6"/>
      <text x="130" y="115" textAnchor="middle" fill={`${ac}33`} fontSize="3.5">Hippodrom</text>
      {/* Sea wall */}
      <rect x="0" y="141" width="175" height="5" fill="rgba(122,96,64,0.3)" stroke={`${ac}44`} strokeWidth="1.5"/>
      <text x="87" y="143.5" textAnchor="middle" fill={`${ac}44`} fontSize="4">SEEMAUER (60km)</text>
      {/* Kerkaporta — fatal */}
      ...
      <text x="12" y="48" textAnchor="middle" fill="#ff4444" fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("kk")}>⚠</text>
      <text x="0" y="54" fill="#cc3322" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("kk")}>Kerkaporta</text>
      <text x="0" y="60" fill="rgba(204,50,34,0.8)" fontSize="3.5" style={{cursor:"pointer"}} onClick={()=>onZone("kk")}>(1453: offen!)</text>
      {/* Golden Horn hint */}
      <path d="M 0 45 Q 60 50 120 45" fill="none" stroke="rgba(68,136,204,0.2)" strokeWidth="2"/>
      <text x="60" y="44" fill="rgba(68,136,204,0.3)" fontSize="4">Goldenes Horn</text>
    </g>
  ),
  ...
  barad_dur: ({ac,sel,onZone}) => (
    <g>
      ...
      {/* Ash clouds */}
      <text x="170" y="30" textAnchor="middle" fill="rgba(80,60,40,0.35)" fontSize="7" fontFamily="serif">MORDOR</text>
      {/* Outer Mordor ring */}
      ...
      <text x="40" y="25" fill={`${ac}33`} fontSize="5" style={{cursor:"pointer"}} onClick={()=>onZone("md")}>Mordor-Festungsring</text>
      {/* Tower walls */}
      ...
      <text x="110" y="107" textAnchor="middle" fill="rgba(255,80,20,0.9)" fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("ey")}>👁</text>
      <text x="110" y="118" textAnchor="middle" fill="rgba(204,68,34,0.7)" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ey")}>AUGE SAURONS</text>
      {/* Cirith Ungol */}
      ...
      <text x="182" y="52" textAnchor="middle" fill="#886633" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("cu")}>⚠ C.Ungol</text>
      {/* Mount Doom */}
      <path d="M 45 160 L 65 120 L 85 160 Z" fill="rgba(180,60,10,0.35)" stroke="rgba(200,80,15,0.5)" strokeWidth="1.5"/>
      <text x="65" y="157" textAnchor="middle" fill="rgba(200,80,15,0.6)" fontSize="5">🌋 Schicksals-</text>
      <text x="65" y="164" textAnchor="middle" fill="rgba(200,80,15,0.6)" fontSize="5">berg</text>
      {/* Compass + title */}
      <text x="200" y="25" textAnchor="middle" fill={`${ac}55`} fontSize="8" fontFamily="serif">N</text>
      <line x1="200" y1="28" x2="200" y2="44" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="195" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">BARAD-DÛR — MORDOR</text>
    </g>
  ),
  ...
  chateau_gaillard: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="178" textAnchor="middle" fill="rgba(68,136,204,0.4)" fontSize="6" fontFamily="serif">Seine</text>
      {/* Rock spur — the cliff */}
      ...
      <text x="110" y="138" textAnchor="middle" fill={`${ac}55`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("op")}>ÄUSSERES VORWERK</text>
      ...
      <text x="110" y="118" textAnchor="middle" fill={`${ac}77`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("me")}>MITTL. ENCEINTE</text>
      <text x="110" y="126" textAnchor="middle" fill={`${ac}55`} fontSize="4.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("me")}>(wellenförmige Mauer)</text>
      {/* Inner ward */}
      ...
      <text x="110" y="108" textAnchor="middle" fill="#e8c860" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("dj")}>DONJON</text>
      ...
      <text x="148" y="90" textAnchor="middle" fill="#ff5555" fontSize="7" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("lt")}>⚠</text>
      <text x="148" y="98" textAnchor="middle" fill="#cc4444" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("lt")}>Latrine!</text>
      ...
      <text x="172" y="78" textAnchor="middle" fill="rgba(204,68,68,0.5)" fontSize="4">Philipps</text>
      <text x="172" y="84" textAnchor="middle" fill="rgba(204,68,68,0.5)" fontSize="4">Soldat →</text>
      {/* Compass + title */}
      ...
      <text x="110" y="198" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">CHÂTEAU GAILLARD — 1196</text>
    </g>
  ),
  ...
  harlech: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="27" y="100" textAnchor="middle" fill="rgba(68,136,204,0.35)" fontSize="5.5" fontFamily="serif" transform="rotate(-90 27 100)">IRISCHE SEE</text>
      ...
      <text x="60" y="22" fill={`${ac}33`} fontSize="4.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("cl2")}>Felsabfall 60m</text>
      ...
      <text x="130" y="103" textAnchor="middle" fill={`${ac}88`} fontSize="6.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("in3")}>HAUPTKURTINE</text>
      {/* Inner courtyard */}
      <rect x="96" y="70" width="68" height="60" rx="1"
        fill="rgba(60,50,25,0.15)" stroke={`${ac}22`} strokeWidth="0.8"/>
      <text x="130" y="103" textAnchor="middle" fill="rgba(100,80,30,0.3)" fontSize="5" fontFamily="serif">Innenhof</text>
      {/* Gatehouse — east */}
      <rect x="176" y="88" width="22" height="25" rx="1"
        fill="rgba(201,168,76,0.2)" stroke={`${ac}88`} strokeWidth="2"/>
      <text x="187" y="103" textAnchor="middle" fill={`${ac}77`} fontSize="4.5" fontFamily="serif">Tor-</text>
      <text x="187" y="109" textAnchor="middle" fill={`${ac}77`} fontSize="4.5" fontFamily="serif">haus</text>
      ...
      <text x="69" y="100" textAnchor="middle" fill="#44aacc" fontSize="4" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("sg")}>SEE-</text>
      <text x="69" y="106" textAnchor="middle" fill="#44aacc" fontSize="4" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("sg")}>TOR</text>
      {/* East side weakness */}
      ...
      <text x="210" y="103" textAnchor="middle" fill="#cc6644" fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("es")}>⚠</text>
      <text x="210" y="110" textAnchor="middle" fill="#cc6644" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("es")}>Ost</text>
      {/* Compass + title */}
      ...
      <text x="120" y="196" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">HARLECH CASTLE</text>
    </g>
  ),
  ...
  himeji: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="35" y="186" fill={`${ac}33`} fontSize="4.5" fontFamily="serif">83 Tore — Irrgarten</text>
      {/* Second ring */}
      ...
      <text x="110" y="80" textAnchor="middle" fill={`${ac}66`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ib")}>INNERES GELÄNDE</text>
      {/* Inner walls */}
      ...
      <text x="110" y="107" textAnchor="middle" fill="#e8d870" fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ts")}>TENSHU</text>
      <text x="110" y="114" textAnchor="middle" fill="#e8d870" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("ts")}>7 Stockwerke</text>
      ...
      <text x="155" y="68" textAnchor="middle" fill="#cc4444" fontSize="4.5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("fr")}>⚠ Feuer</text>
      {/* Wind direction arrow */}
      <path d="M 178 40 L 155 55" stroke="#cc4444" strokeWidth="1.5" strokeDasharray="2,2" opacity="0.5"/>
      <text x="185" y="38" fill="rgba(204,68,68,0.45)" fontSize="4.5">Wind →</text>
      ...
      <text x="110" y="198" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">HIMEJI-JŌ — WEISSER REIHER</text>
    </g>
  ),
  ...
  alamut: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="145" textAnchor="middle" fill={`${ac}55`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("mt")}>ALBORZ-GEBIRGE</text>
      <text x="110" y="153" textAnchor="middle" fill={`${ac}44`} fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("mt")}>2100m Höhe</text>
      ...
      <text x="110" y="98" textAnchor="middle" fill={`${ac}88`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("kb")}>KERNBURG</text>
      ...
      <text x="110" y="111" textAnchor="middle" fill="rgba(150,100,200,0.4)" fontSize="4.5">Bibliothek</text>
      ...
      <text x="110" y="170" textAnchor="middle" fill="#cc4444" fontSize="6" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("bp")}>⚠</text>
      <text x="110" y="178" textAnchor="middle" fill="#cc4444" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("bp")}>Bergpfad</text>
      {/* Attacking force at bottom */}
      <text x="110" y="195" textAnchor="middle" fill="rgba(204,68,68,0.35)" fontSize="4.5">▲ Mongolisches Heer (unten)</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="198" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">BURG ALAMUT — 2100m</text>
    </g>
  ),
  ...
  coucy: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="175" y="75" fill="#446688" fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("wg")}>Wassergraben</text>
      {/* Outer ring wall */}
      ...
      <text x="165" y="108" fill={`${ac}44`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ar")}>Äußere</text>
      <text x="165" y="115" fill={`${ac}44`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ar")}>Ringmauer</text>
      {/* Inner ring wall */}
      ...
      <text x="110" y="80" textAnchor="middle" fill={`${ac}77`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ir2")}>INNERE RINGMAUER</text>
      ...
      <text x="110" y="100" textAnchor="middle" fill="#e8c860" fontSize="6.5" fontFamily="serif" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("dn")}>DONJON</text>
      <text x="110" y="109" textAnchor="middle" fill="#e8c860" fontSize="5" style={{cursor:"pointer"}} onClick={()=>onZone("dn")}>54m · 31m ∅</text>
      <text x="110" y="117" textAnchor="middle" fill="rgba(232,200,96,0.6)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("dn")}>7m Mauern</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="198" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">CHÂTEAU DE COUCY</text>
    </g>
  ),
  ...
  babylon: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="24" y="100" textAnchor="middle" fill="#4488cc" fontSize="6" fontFamily="serif" transform="rotate(90 24 100)">EUPHRAT</text>
      ...
      <text x="22" y="148" textAnchor="middle" fill="#ff4444" fontSize="6" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("fb")}>⚠</text>
      <text x="22" y="157" textAnchor="middle" fill="#cc4444" fontSize="4" style={{cursor:"pointer"}} onClick={()=>onZone("fb")}>Flussbett</text>
      {/* Outer wall */}
      ...
      <text x="135" y="22" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ow")}>ÄUSSERE MAUER (7m)</text>
      {/* Inner wall */}
      ...
      <text x="132" y="40" textAnchor="middle" fill={`${ac}77`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("iw")}>INNERE MAUER (7m) — STREITWAGEN-WEG</text>
      {/* Processional Way */}
      <line x1="132" y1="174" x2="132" y2="26" stroke="rgba(201,168,76,0.15)" strokeWidth="4"/>
      <text x="132" y="115" textAnchor="middle" fill="rgba(201,168,76,0.25)" fontSize="4.5" transform="rotate(90 132 115)">Prozessionsstraße</text>
      {/* Hanging Gardens */}
      ...
      <text x="169" y="58" textAnchor="middle" fill="#88aa44" fontSize="4.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("hg")}>Hängende</text>
      <text x="169" y="65" textAnchor="middle" fill="#88aa44" fontSize="4.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("hg")}>Gärten</text>
      <text x="169" y="72" textAnchor="middle" fill="#88aa44" fontSize="7" style={{cursor:"pointer"}} onClick={()=>onZone("hg")}>🌿</text>
      {/* Ishtar Gate — north */}
      <rect x="118" y="24" width="28" height="12" rx="1" fill={`${ac}55`} stroke={`${ac}99`} strokeWidth="1.5"/>
      <text x="132" y="33" textAnchor="middle" fill={`${ac}88`} fontSize="4.5">Ischtar-Tor</text>
      {/* Palace */}
      <rect x="76" y="40" width="62" height="45" rx="2" fill="rgba(201,168,76,0.1)" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="107" y="66" textAnchor="middle" fill={`${ac}55`} fontSize="5" fontFamily="serif">Palast</text>
      {/* Marduk temple */}
      <rect x="100" y="105" width="42" height="52" rx="2" fill="rgba(201,168,76,0.09)" stroke={`${ac}33`} strokeWidth="0.8"/>
      <text x="121" y="135" textAnchor="middle" fill={`${ac}44`} fontSize="4.5" fontFamily="serif">Esagila</text>
      <text x="121" y="143" textAnchor="middle" fill={`${ac}33`} fontSize="4">(Marduk-Tempel)</text>
      {/* Compass + title */}
      ...
      <text x="135" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">BABYLON — 605 v.Chr.</text>
    </g>
  ),
  ...
  alhambra: ({ac,sel,onZone}) => (
    <g>
      ...
      <circle cx="10" cy="35" r="7"
        ...
        style={{cursor:"pointer"}} onClick={()=>onZone("aq")}/>
      <text x="6" y="25" fill="#4488cc" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("aq")}>Acequia</text>
      <text x="6" y="32" fill="#4488cc" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("aq")}>(Sierra↓)</text>
      {/* Cliff/mountain zone */}
      ...
      <text x="30" y="155" fill={`${ac}33`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("cl3")}>Kliff &</text>
      <text x="30" y="163" fill={`${ac}33`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("cl3")}>Gebirge</text>
      ...
      <text x="70" y="130" textAnchor="middle" fill="#aa7733" fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("al")}>ALCAZABA</text>
      <text x="70" y="139" textAnchor="middle" fill="rgba(170,119,51,0.6)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("al")}>(Militärburg)</text>
      ...
      <text x="135" y="122" textAnchor="middle" fill={`${ac}55`} fontSize="4.5">Löwenhof</text>
      {/* Courtyard of the Myrtles */}
      <rect x="108" y="142" width="54" height="18" rx="1" fill="rgba(68,136,204,0.12)" stroke="#4488cc" strokeWidth="0.6"/>
      <text x="135" y="153" textAnchor="middle" fill="rgba(68,136,204,0.5)" fontSize="4.5">Myrtenhof (Bassin)</text>
      <text x="135" y="128" textAnchor="middle" fill={`${ac}88`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("py")}>NASRIDEN-</text>
      <text x="135" y="137" textAnchor="middle" fill={`${ac}88`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("py")}>PALÄSTE</text>
      ...
      <text x="193" y="112" textAnchor="middle" fill="rgba(120,160,60,0.6)" fontSize="5" fontFamily="serif">Genera-</text>
      <text x="193" y="120" textAnchor="middle" fill="rgba(120,160,60,0.6)" fontSize="5" fontFamily="serif">life</text>
      {/* Main gate */}
      <rect x="95" y="167" width="16" height="8" rx="1" fill={`${ac}55`} opacity="0.8"/>
      <text x="103" y="183" textAnchor="middle" fill={`${ac}55`} fontSize="4.5" fontFamily="serif">Haupttor</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">ALHAMBRA — GRANADA</text>
    </g>
  ),
  ...
  mehrangarh: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="178" textAnchor="middle" fill="rgba(80,100,160,0.25)" fontSize="5.5" fontFamily="serif">Jodhpur — Blaue Stadt</text>
      ...
      <text x="30" y="130" fill="rgba(140,90,40,0.4)" fontSize="5" fontFamily="serif" transform="rotate(-72 30 130)">122m FELS</text>
      {/* Rock base platform */}
      ...
      <text x="110" y="115" textAnchor="middle" fill={`${ac}88`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("hw")}>HAUPTMAUER</text>
      <text x="110" y="123" textAnchor="middle" fill="rgba(204,136,51,0.6)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("hw")}>bis 36m hoch</text>
      ...
      <text x="130" y="140" fill={`${ac}66`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("pg")}>← 7 Tore</text>
      {/* Palace complex */}
      ...
      <text x="110" y="96" textAnchor="middle" fill="#e8c860" fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("pa")}>PALASTKOMPLEX</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">MEHRANGARH — JODHPUR</text>
    </g>
  ),
  ...
  akkon: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="27" y="35" textAnchor="middle" fill="rgba(68,136,204,0.35)" fontSize="5.5" fontFamily="serif">MITTELMEER</text>
      {/* Sea walls */}
      ...
      <text x="29" y="55" textAnchor="middle" fill="#4488cc" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("sm")}>SEEMAUER</text>
      {/* Harbor marker */}
      <ellipse cx="28" cy="90" rx="20" ry="25"
        fill="rgba(25,65,125,0.35)" stroke="rgba(40,90,170,0.3)" strokeWidth="1"/>
      <text x="28" y="90" textAnchor="middle" fill="rgba(68,136,204,0.5)" fontSize="5" fontFamily="serif">Hafen</text>
      {/* Port critical zone */}
      <circle cx="48" cy="92" r="9"
        fill={sel==="ht"?"rgba(204,68,68,0.55)":"rgba(204,68,68,0.22)"}
        stroke="#cc4444" strokeWidth="2"
        style={{cursor:"pointer"}} onClick={()=>onZone("ht")}/>
      <text x="48" y="97" textAnchor="middle" fill="#cc4444" fontSize="4" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("ht")}>⚠</text>
      {/* Outer city wall */}
      ...
      <text x="133" y="110" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ia")}>INNERE STADTMAUER</text>
      {/* Johanniter Citadel */}
      ...
      <text x="107" y="110" textAnchor="middle" fill="#e8c860" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ja")}>JOHANNITER-</text>
      <text x="107" y="118" textAnchor="middle" fill="#e8c860" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ja")}>CITADEL</text>
      {/* Templar quarter */}
      <rect x="144" y="148" width="44" height="36" rx="2" fill="rgba(180,80,80,0.12)" stroke="rgba(180,80,80,0.3)" strokeWidth="1"/>
      <text x="166" y="168" textAnchor="middle" fill="rgba(180,80,80,0.5)" fontSize="4.5" fontFamily="serif">Templer</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="135" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">AKKON — 1291</text>
    </g>
  ),
  ...
  mont_michel: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="20" y="40" fill="rgba(68,136,204,0.4)" fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ti")}>GEZEITEN</text>
      <text x="20" y="48" fill="rgba(68,136,204,0.4)" fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ti")}>2× tägl.</text>
      {/* Quicksand zone */}
      ...
      <text x="182" y="80" fill="rgba(68,100,80,0.45)" fontSize="4.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("qs")}>Treibsand</text>
      {/* The causeway path — only approach */}
      <path d="M 110 195 L 110 160 L 108 145" stroke={`${ac}44`} strokeWidth="3" strokeDasharray="5,3"/>
      <text x="125" y="182" fill={`${ac}44`} fontSize="4.5">Damm</text>
      {/* The rock */}
      ...
      <text x="110" y="127" textAnchor="middle" fill={`${ac}77`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ws2")}>STADTMAUERN</text>
      {/* Abbey — the summit */}
      ...
      <text x="110" y="90" textAnchor="middle" fill={`${ac}99`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ab")}>ABTEI</text>
      {/* Compass + title */}
      <text x="194" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="194" y1="25" x2="194" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="198" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">MONT SAINT-MICHEL</text>
    </g>
  ),
  ...
  caernarvon: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="172" textAnchor="middle" fill="rgba(68,136,204,0.35)" fontSize="5.5" fontFamily="serif">Fluss Seiont</text>
      {/* Town — north */}
      <rect x="0" y="0" width="220" height="50" fill="rgba(38,50,28,0.15)"/>
      <text x="110" y="25" textAnchor="middle" fill="rgba(100,120,60,0.3)" fontSize="5.5" fontFamily="serif">Caernarfon — Stadt</text>
      ...
      <text x="35" y="95" textAnchor="middle" fill="#9988bb" fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("et")}>Eagle</text>
      <text x="35" y="103" textAnchor="middle" fill="#9988bb" fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("et")}>Tower</text>
      ...
      <text x="110" y="45" textAnchor="middle" fill={`${ac}88`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("kg")}>KÖNIGSTOR</text>
      ...
      <text x="110" y="175" textAnchor="middle" fill="#4488cc" fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("wg2")}>Hafentor (Seeversorgung)</text>
      {/* Well */}
      <circle cx="110" cy="105" r="6" fill="rgba(68,136,204,0.2)" stroke="#4488cc" strokeWidth="0.8"/>
      <text x="110" y="108" textAnchor="middle" fill="rgba(68,136,204,0.5)" fontSize="4">Brunnen</text>
      {/* Compass + title */}
      ...
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">CAERNARVON CASTLE</text>
    </g>
  ),
  ...
  windsor: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="180" textAnchor="middle" fill="rgba(68,136,204,0.35)" fontSize="5.5" fontFamily="serif">Themse</text>
      {/* Hill */}
      ...
      <text x="63" y="112" textAnchor="middle" fill={`${ac}66`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("la")}>LOWER</text>
      <text x="63" y="120" textAnchor="middle" fill={`${ac}66`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("la")}>WARD</text>
      {/* St George's Chapel */}
      <rect x="30" y="72" width="65" height="30" rx="2" fill="rgba(201,168,76,0.12)" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="62" y="91" textAnchor="middle" fill={`${ac}55`} fontSize="4.5">St. George's Chapel</text>
      ...
      <text x="110" y="107" textAnchor="middle" fill={`${ac}99`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("rn")}>ROUND</text>
      <text x="110" y="115" textAnchor="middle" fill={`${ac}99`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("rn")}>TOWER</text>
      {/* Upper Ward */}
      ...
      <text x="157" y="112" textAnchor="middle" fill={`${ac}66`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ua")}>UPPER</text>
      <text x="157" y="120" textAnchor="middle" fill={`${ac}66`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ua")}>WARD</text>
      {/* State apartments */}
      <rect x="124" y="70" width="66" height="35" rx="2" fill="rgba(201,168,76,0.1)" stroke={`${ac}33`} strokeWidth="0.8"/>
      <text x="157" y="91" textAnchor="middle" fill={`${ac}44`} fontSize="4.5">Staatsgemächer</text>
      {/* Compass + title */}
      ...
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">WINDSOR CASTLE</text>
    </g>
  ),
  ...
  bodiam: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="25" y="65" fill="rgba(68,136,204,0.4)" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("wm")}>WASSER-</text>
      <text x="25" y="73" fill="rgba(68,136,204,0.4)" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("wm")}>GRABEN</text>
      ...
      <text x="110" y="107" textAnchor="middle" fill="rgba(68,136,204,0.5)" fontSize="3.5">Brunnen</text>
      ...
      <text x="110" y="158" textAnchor="middle" fill={`${ac}88`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("bb")}>Barba-</text>
      <text x="110" y="165" textAnchor="middle" fill={`${ac}88`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("bb")}>kane</text>
      {/* Postern gate north */}
      <rect x="96" y="64" width="28" height="10" rx="1" fill={`${ac}44`} stroke={`${ac}77`} strokeWidth="1.5"/>
      <text x="110" y="60" textAnchor="middle" fill={`${ac}55`} fontSize="4.5">Nordtor</text>
      {/* Compass + title */}
      <text x="194" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="194" y1="25" x2="194" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">BODIAM CASTLE</text>
    </g>
  ),
  ...
  rhodes: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="190" y="102" textAnchor="middle" fill="rgba(68,136,204,0.4)" fontSize="5" fontFamily="serif">Hafen</text>
      {/* Island terrain */}
      ...
      <text x="163" y="35" fill="#4488cc" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("sm2")}>Seemauer</text>
      ...
      <text x="93" y="42" textAnchor="middle" fill="rgba(60,140,90,0.6)" fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("gv")}>GRABEN — 17m TIEF (aus Fels gehauen)</text>
      {/* Outer bastions */}
      ...
      <text x="90" y="120" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("bm")}>BASTION-MAUERSYSTEM</text>
      ...
      <text x="58" y="110" textAnchor="middle" fill={`${ac}88`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("cz")}>COLLACHIUM</text>
      <text x="58" y="119" textAnchor="middle" fill={`${ac}55`} fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("cz")}>(Johanniterquartier)</text>
      ...
      <text x="90" y="46" textAnchor="middle" fill="#cc4433" fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("ko")}>⚠</text>
      <text x="90" y="36" textAnchor="middle" fill="rgba(204,68,53,0.5)" fontSize="4">Kanonen</text>
      {/* Compass + title */}
      <text x="10" y="22" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="10" y1="25" x2="10" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="90" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">RHODOS — JOHANNITERBURG</text>
    </g>
  ),
  ...
  persepolis: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="25" y="96" fill={`${ac}44`} fontSize="4.5">Terrasse — 12m hoch</text>
      {/* Terrace fill zone */}
      ...
      <text x="25" y="130" fill={`${ac}44`} fontSize="4" transform="rotate(90 25 130)">Treppe</text>
      {/* Gate of All Nations */}
      ...
      <text x="110" y="113" textAnchor="middle" fill={`${ac}88`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("gp")}>ALLER-VÖLKER-TOR</text>
      ...
      <text x="80" y="150" textAnchor="middle" fill="#ddaa44" fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ap")}>APADANA</text>
      <text x="80" y="159" textAnchor="middle" fill="rgba(221,170,68,0.6)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("ap")}>(Thronsaal)</text>
      {/* Treasury */}
      ...
      <text x="160" y="155" textAnchor="middle" fill="#ffcc33" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("tr")}>SCHATZKAMMER</text>
      <text x="160" y="164" textAnchor="middle" fill="rgba(255,200,50,0.55)" fontSize="5" style={{cursor:"pointer"}} onClick={()=>onZone("tr")}>💰</text>
      {/* Palace of Darius */}
      <rect x="40" y="178" width="50" height="15" rx="1" fill="rgba(201,168,76,0.1)" stroke={`${ac}44`} strokeWidth="0.8"/>
      <text x="65" y="188" textAnchor="middle" fill={`${ac}44`} fontSize="4.5">Darius-Palast</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="97" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">PERSEPOLIS — 518 v.Chr.</text>
    </g>
  ),
  ...
  great_wall: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="30" textAnchor="middle" fill="rgba(150,120,55,0.28)" fontSize="7" fontFamily="serif">GOBI-WÜSTE — MONGOLISCHES REICH</text>
      {/* Chinese heartland — south */}
      <rect x="0" y="130" width="220" height="70" fill="rgba(48,68,35,0.2)"/>
      <text x="110" y="170" textAnchor="middle" fill="rgba(60,90,40,0.28)" fontSize="7" fontFamily="serif">MING-CHINA</text>
      ...
      <text x="110" y="100" textAnchor="middle" fill={`${ac}ee`} fontSize="6.5" fontFamily="serif" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("mm")}>GROSSE MAUER</text>
      <text x="110" y="109" textAnchor="middle" fill={`${ac}77`} fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("mm")}>9m hoch · 5m breit · 20.000km</text>
      ...
      <text x="110" y="90" textAnchor="middle" fill={`${ac}99`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("jg")}>JIAYUGUAN-</text>
      <text x="110" y="98" textAnchor="middle" fill={`${ac}99`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("jg")}>FORT</text>
      ...
      <text x="5" y="43" textAnchor="middle" fill="#cc4444" fontSize="6" fontWeight="bold"
        style={{cursor:"pointer"}} onClick={()=>onZone("ds")}>⚠</text>
      <text x="5" y="34" fill="rgba(204,68,68,0.5)" fontSize="4.5">Wüsten-</text>
      <text x="5" y="41" fill="rgba(204,68,68,0.5)" fontSize="4.5">umgehung</text>
      {/* Signal fire system */}
      {[35,62,89,116,143,170].map((x,i)=>(
        <text key={i} x={x} y="75" textAnchor="middle" fill="rgba(255,150,30,0.4)" fontSize="6">🔥</text>
      ))}
      {/* Compass + title */}
      ...
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">GROSSE MAUER — JIAYUGUAN</text>
    </g>
  ),
  ...
  angkor: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="20" y="40" fill="rgba(40,120,90,0.45)" fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("mg")}>10km WASSERGRABEN</text>
      {/* Jungle zone */}
      <ellipse cx="110" cy="103" rx="78" ry="68"
        ...
        style={{cursor:"pointer"}} onClick={()=>onZone("dj")}/>
      <text x="170" y="70" fill="rgba(40,100,32,0.45)" fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("dj")}>Dschungel</text>
      {/* Temple complex */}
      <ellipse cx="110" cy="103" rx="55" ry="48"
      ...
      <text x="110" y="80" textAnchor="middle" fill={`${ac}66`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("tp")}>TEMPELGALERIEN</text>
      ...
      <text x="110" y="100" textAnchor="middle" fill={`${ac}99`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ak")}>ANGKOR</text>
      <text x="110" y="108" textAnchor="middle" fill={`${ac}99`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ak")}>THOM</text>
      {/* 4 causeways to city */}
      ...
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">ANGKOR — KHMER-REICH</text>
    </g>
  ),
  ...
  gondolin: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="25" y="112" fill={`${ac}33`} fontSize="4.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ew")}>Ered Wethrin</text>
      ...
      <text x="110" y="103" textAnchor="middle" fill="#ddeeff" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("tk")}>TURGONS</text>
      <text x="110" y="111" textAnchor="middle" fill="#ddeeff" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("tk")}>TURM</text>
      ...
      <text x="110" y="46" textAnchor="middle" fill="#8888cc" fontSize="6" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("gs")}>⚠</text>
      <text x="110" y="54" textAnchor="middle" fill="#8888cc" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("gs")}>DAS GEHEIMNIS</text>
      <text x="110" y="62" textAnchor="middle" fill="rgba(136,136,204,0.6)" fontSize="4">(Maeglin!)</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">GONDOLIN — VERBORGENE STADT</text>
    </g>
  ),
  ...
  erebor: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="170" textAnchor="middle" fill="#8b6914" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("fg")}>VORDERTOR</text>
      <text x="110" y="178" textAnchor="middle" fill="rgba(139,105,20,0.5)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("fg")}>(einziger Zugang)</text>
      ...
      <text x="110" y="118" textAnchor="middle" fill={`${ac}77`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("hh")}>GROSSE HALLEN</text>
      {/* The treasure chamber */}
      <ellipse cx="110" cy="88" rx="28" ry="22"
        fill={sel==="sm2"?"rgba(232,200,32,0.45)":"rgba(232,200,32,0.15)"}
        stroke="#e8c820" strokeWidth="3"
        style={{cursor:"pointer"}} onClick={()=>onZone("sm2")}/>
      <text x="110" y="86" textAnchor="middle" fill="#e8c820" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("sm2")}>SCHATZ-</text>
      <text x="110" y="94" textAnchor="middle" fill="#e8c820" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("sm2")}>KAMMER</text>
      ...
      <text x="172" y="98" textAnchor="middle" fill="#cc7733" fontSize="5.5" style={{cursor:"pointer"}} onClick={()=>onZone("sd")}>⚠</text>
      <text x="172" y="107" textAnchor="middle" fill="#cc7733" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("sd")}>Seiten-</text>
      <text x="172" y="114" textAnchor="middle" fill="#cc7733" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("sd")}>tür</text>
      {/* Compass + title */}
      ...
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">EREBOR — DER EINSAME BERG</text>
    </g>
  ),
  ...
  isengard: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="180" y="55" textAnchor="middle" fill="rgba(40,100,28,0.55)" fontSize="6" fontFamily="serif">FANGORN</text>
      <text x="180" y="63" textAnchor="middle" fill="rgba(40,100,28,0.45)" fontSize="5" fontFamily="serif">WALD</text>
      ...
      <text x="145" y="70" textAnchor="middle" fill="#4a7a33" fontSize="6" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("fn")}>⚠</text>
      <text x="145" y="79" textAnchor="middle" fill="#4a7a33" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("fn")}>Ents!</text>
      {/* River Isen — runs west */}
      <path d="M 0 155 Q 55 148 110 155 Q 165 162 220 155" fill="none" stroke="rgba(68,136,204,0.3)" strokeWidth="3"/>
      <text x="55" y="150" fill="rgba(68,136,204,0.3)" fontSize="4.5">Fluss Isen</text>
      ...
      <text x="170" y="45" fill={`${ac}44`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("rw")}>Ringwall</text>
      ...
      <text x="105" y="116" textAnchor="middle" fill={`${ac}77`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("fk")}>FABRIKEN</text>
      <text x="105" y="124" textAnchor="middle" fill="rgba(100,120,100,0.5)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("fk")}>(Uruk-hai Produktion)</text>
      ...
      <text x="105" y="72" textAnchor="middle" fill="#667766" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ot")}>ORTHANC</text>
      <text x="105" y="80" textAnchor="middle" fill="rgba(102,119,102,0.6)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("ot")}>(unzerstörbar)</text>
      {/* Dam / river control */}
      <rect x="0" y="148" width="25" height="14" rx="1" fill="rgba(68,136,204,0.25)" stroke="#4488cc" strokeWidth="1.5"/>
      <text x="12" y="158" textAnchor="middle" fill="rgba(68,136,204,0.5)" fontSize="4">Damm</text>
      {/* Compass + title */}
      <text x="16" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="16" y1="25" x2="16" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="105" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">ISENGARD — NAN CURUNÍR</text>
    </g>
  ),
  ...
  minas_morgul: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="180" textAnchor="middle" fill="rgba(180,200,80,0.2)" fontSize="5" fontFamily="serif">Vergiftetes Tal — weißes Licht</text>
      ...
      <text x="110" y="50" textAnchor="middle" fill="#5566aa" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("mw")}>MONDWEISSE MAUERN</text>
      {/* Nazgûl Tower */}
      ...
      <text x="110" y="75" textAnchor="middle" fill="#7777cc" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("na")}>NAZGÛL-</text>
      <text x="110" y="83" textAnchor="middle" fill="#7777cc" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("na")}>TURM</text>
      ...
      <text x="110" y="64" textAnchor="middle" fill={`${ac}99`} fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("hk")}>HEXEN-</text>
      <text x="110" y="72" textAnchor="middle" fill={`${ac}99`} fontSize="5" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("hk")}>KÖNIG</text>
      ...
      <text x="110" y="135" textAnchor="middle" fill="rgba(120,160,50,0.4)" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("mt3")}>Vergiftetes Tal</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="196" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">MINAS MORGUL</text>
    </g>
  ),
    ...
    <g>
      ...
      {[[60,5],[110,0],[160,5]].map(([cx,cy],i)=>(
        <g key={i}>
          ...
          <text x={cx} y={cy+8} textAnchor="middle" fill="rgba(220,80,20,0.5)" fontSize="6">🌋</text>
        </g>
      ))}
      ...
      <text x="110" y="148" textAnchor="middle" fill={`${ac}66`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("df")}>TIEFE KERKER</text>
      <text x="110" y="157" textAnchor="middle" fill={`${ac}44`} fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("df")}>(unzählige Stockwerke tief)</text>
      {/* Entrance gates */}
      ...
      <text x="110" y="126" textAnchor="middle" fill="#cc3322" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("mk")}>MORGOTHS THRON</text>
      ...
      <text x="75" y="146" textAnchor="middle" fill="rgba(200,60,20,0.5)" fontSize="4">Balrog</text>
      <text x="145" y="146" textAnchor="middle" fill="rgba(200,60,20,0.5)" fontSize="4">Balrog</text>
      {/* Morgoth himself */}
      <circle cx="110" cy="130" r="14"
        fill="rgba(153,34,17,0.35)" stroke="#992211" strokeWidth="2.5"
        style={{cursor:"pointer"}} onClick={()=>onZone("mk")}/>
      <text x="110" y="128" textAnchor="middle" fill="#cc3322" fontSize="7" style={{cursor:"pointer"}} onClick={()=>onZone("mk")}>👑</text>
      <text x="110" y="139" textAnchor="middle" fill="rgba(153,34,17,0.7)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("mk")}>MORGOTH</text>
      {/* Thangorodrim zone */}
      ...
      <text x="15" y="108" fill={`${ac}33`} fontSize="4.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("th")}>THANGORODRIM</text>
      {/* Compass + title */}
      ...
      <text x="110" y="196" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">ANGBAND — ERSTES ZEITALTER</text>
    </g>
  ),
  ...
  khazad_dum: ({ac,sel,onZone}) => (
    <g>
      ...
      {['Obere Ebene','Mittlere Ebene','Tiefe Ebene','Tiefste Ebene'].map((l,i)=>(
        <text key={l} x="8" y={68+i*35} fill={`${ac}${['44','33','28','1e'][i]}`} fontSize="4.5" fontFamily="serif">{l}</text>
      ))}
      {/* Nebelgebirge zone */}
      <rect x="0" y="0" width="220" height="55"
        fill={sel==="ng"?"rgba(55,60,80,0.35)":"transparent"}
        style={{cursor:"pointer"}} onClick={()=>onZone("ng")}/>
      <text x="110" y="30" textAnchor="middle" fill={`${ac}33`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ng")}>NEBELGEBIRGE — GANZER BERG</text>
      ...
      {[50,75,100,125,150,175].map((x,i)=>(
        <rect key={i} x={x-4} y="60" width="8" height="56" rx="1"
          fill={`${ac}22`} stroke={`${ac}44`} strokeWidth="0.6"/>
      ))}
      <text x="110" y="92" textAnchor="middle" fill={`${ac}66`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("gh")}>GROSSE HALLEN</text>
      <text x="110" y="101" textAnchor="middle" fill="rgba(120,100,60,0.4)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("gh")}>(von Orks besetzt)</text>
      ...
      <text x="110" y="148" textAnchor="middle" fill="rgba(10,10,20,0.0)" fontSize="5">∞</text>
      <text x="110" y="123" textAnchor="middle" fill="#664422" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("bt")}>BRÜCKE von</text>
      <text x="110" y="131" textAnchor="middle" fill="#664422" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("bt")}>KHAZAD-DÛM</text>
      ...
      <text x="110" y="169" textAnchor="middle" fill="#cc3322" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("db")}>DURINS BANE</text>
      <text x="110" y="178" textAnchor="middle" fill="rgba(200,60,20,0.6)" fontSize="8" style={{cursor:"pointer"}} onClick={()=>onZone("db")}>🔥</text>
      <text x="110" y="188" textAnchor="middle" fill="rgba(200,60,20,0.4)" fontSize="4.5">(BALROG — wartet)</text>
      {/* East & West gates */}
      ...
      <text x="6" y="95" textAnchor="middle" fill={`${ac}55`} fontSize="4" transform="rotate(-90 6 95)">W-Tor</text>
      <text x="214" y="95" textAnchor="middle" fill={`${ac}55`} fontSize="4" transform="rotate(90 214 95)">O-Tor</text>
      {/* Compass + title */}
      ...
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">KHAZAD-DÛM — MORIA</text>
    </g>
  ),
  ...
  edoras: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="30" y="178" fill="rgba(100,90,35,0.25)" fontSize="6" fontFamily="serif">ROHAN — RIDDERMARK</text>
      {/* The hill */}
      ...
      <text x="155" y="80" fill={`${ac}44`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("pw")}>Palisade</text>
      ...
      <text x="110" y="100" textAnchor="middle" fill="#cc9933" fontSize="6.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("md2")}>MEDUSELD</text>
      <text x="110" y="110" textAnchor="middle" fill="rgba(204,153,51,0.7)" fontSize="5" style={{cursor:"pointer"}} onClick={()=>onZone("md2")}>(Goldene Halle)</text>
      {/* Théoden's throne */}
      <rect x="99" y="84" width="22" height="15" rx="1" fill="rgba(232,200,80,0.2)" stroke="rgba(232,200,80,0.4)" strokeWidth="0.8"/>
      <text x="110" y="94" textAnchor="middle" fill="rgba(232,200,80,0.5)" fontSize="4">Thron</text>
      ...
      <text x="40" y="102" textAnchor="middle" fill="#cc9933" fontSize="7" style={{cursor:"pointer"}} onClick={()=>onZone("ro")}>🐎</text>
      <text x="40" y="116" textAnchor="middle" fill="#cc9933" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("ro")}>Rohirrim</text>
      {/* Main gate */}
      <rect x="96" y="152" width="28" height="10" rx="1" fill={`${ac}44`} stroke={`${ac}77`} strokeWidth="1.5"/>
      <text x="110" y="168" textAnchor="middle" fill={`${ac}55`} fontSize="4.5" fontFamily="serif">Haupttor</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">EDORAS — GOLDENE HALLE</text>
    </g>
  ),
  ...
  winterfell: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="185" textAnchor="middle" fill="rgba(180,195,210,0.2)" fontSize="6" fontFamily="serif">DER NORDEN — WESTEROS</text>
      ...
      <text x="100" y="178" textAnchor="middle" fill="rgba(180,100,40,0.3)" fontSize="4.5">Heiße Quellen (wärmt Mauern)</text>
      {/* Outer ward */}
      ...
      <text x="150" y="38" fill={`${ac}44`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("ow3")}>Äußerer Ring</text>
      {/* Inner ward */}
      ...
      <text x="110" y="75" textAnchor="middle" fill={`${ac}77`} fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("iw3")}>INNERER RING</text>
      ...
      <text x="110" y="95" textAnchor="middle" fill={`${ac}77`} fontSize="5.5" fontFamily="serif">GREAT KEEP</text>
      {/* Godswood */}
      <circle cx="152" cy="110" r="18" fill="rgba(22,55,22,0.3)" stroke="rgba(30,80,30,0.35)" strokeWidth="1.5"/>
      <text x="152" y="107" textAnchor="middle" fill="rgba(40,100,40,0.5)" fontSize="5" fontFamily="serif">Gods-</text>
      <text x="152" y="115" textAnchor="middle" fill="rgba(40,100,40,0.5)" fontSize="5" fontFamily="serif">wood</text>
      ...
      <text x="68" y="131" textAnchor="middle" fill="#cc7744" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("hq")}>Quellen</text>
      ...
      <text x="110" y="160" textAnchor="middle" fill="#556677" fontSize="5.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("cr")}>⚠ CRYPTEN (Untote!)</text>
      {/* Main gate — south */}
      <rect x="96" y="168" width="28" height="10" rx="1" fill={`${ac}44`} stroke={`${ac}77`} strokeWidth="1.5"/>
      <text x="110" y="183" textAnchor="middle" fill={`${ac}55`} fontSize="4.5" fontFamily="serif">Haupttor</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="197" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">WINTERFELL</text>
    </g>
  ),
  ...
  harrenhal: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="185" y="160" textAnchor="middle" fill="rgba(40,90,160,0.25)" fontSize="4.5">Götterauge</text>
      ...
      <text x="140" y="32" fill={`${ac}55`} fontSize="5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("mw2")}>RIESENMAUERN</text>
      ...
      {[
        ...
        <g key={i} style={{cursor:"pointer"}} onClick={()=>onZone("mt3")}>
          ...
          <text x={x} y={y+5} textAnchor="middle" fill="rgba(140,80,30,0.55)" fontSize="4.5" fontFamily="serif">{label}</text>
          {melted&&<text x={x} y={y-2} textAnchor="middle" fill="rgba(200,80,20,0.4)" fontSize="5">🔥</text>}
        </g>
      ))}
      ...
      <text x="102" y="100" textAnchor="middle" fill={`${ac}44`} fontSize="6" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("dk")}>DUNKLE KAMMERN</text>
      <text x="102" y="110" textAnchor="middle" fill={`${ac}33`} fontSize="5" style={{cursor:"pointer"}} onClick={()=>onZone("dk")}>(labyrinthartig)</text>
      {/* The curse marker */}
      ...
      <text x="102" y="137" textAnchor="middle" fill="#cc3322" fontSize="6" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("cr2")}>⚠</text>
      <text x="102" y="148" textAnchor="middle" fill="#882211" fontSize="5" style={{cursor:"pointer"}} onClick={()=>onZone("cr2")}>DER FLUCH</text>
      {/* Main gate */}
      <rect x="88" y="181" width="28" height="10" rx="1" fill={`${ac}44`} stroke={`${ac}77`} strokeWidth="1.5"/>
      <text x="102" y="196" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">HARRENHAL</text>
    </g>
  ),
  ...
  kings_landing: ({ac,sel,onZone}) => (
    <g>
      ...
      <text x="110" y="172" textAnchor="middle" fill="rgba(40,90,170,0.35)" fontSize="6" fontFamily="serif">SCHWARZWASSER-BUCHT</text>
      {/* City terrain */}
      ...
      <text x="110" y="5" textAnchor="middle" fill={`${ac}55`} fontSize="4.5">King's Gate</text>
      {/* City streets */}
      ...
      <text x="102" y="58" textAnchor="middle" fill="#cc4422" fontSize="6.5" fontFamily="serif" style={{cursor:"pointer"}} onClick={()=>onZone("rb")}>ROTE BURG</text>
      <text x="102" y="68" textAnchor="middle" fill="rgba(200,80,50,0.5)" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("rb")}>(Thronsaal)</text>
      {/* Sept of Baelor */}
      <ellipse cx="168" cy="55" rx="18" ry="14" fill="rgba(200,200,180,0.12)" stroke="rgba(200,200,180,0.25)" strokeWidth="1"/>
      <text x="168" y="58" textAnchor="middle" fill="rgba(200,200,180,0.35)" fontSize="4.5">Sept</text>
      {/* Dragonpit — north */}
      <ellipse cx="45" cy="55" rx="20" ry="16" fill="rgba(80,60,30,0.2)" stroke="rgba(100,75,35,0.3)" strokeWidth="1"/>
      <text x="45" y="57" textAnchor="middle" fill="rgba(140,100,40,0.4)" fontSize="4.5">Drachengrube</text>
      ...
      <text x="80" y="108" textAnchor="middle" fill="#44cc44" fontSize="6" fontWeight="bold" style={{cursor:"pointer"}} onClick={()=>onZone("wf")}>⚠</text>
      <text x="80" y="117" textAnchor="middle" fill="#44cc44" fontSize="4.5" style={{cursor:"pointer"}} onClick={()=>onZone("wf")}>WILDFIRE</text>
      {/* Compass + title */}
      <text x="196" y="22" textAnchor="middle" fill={`${ac}66`} fontSize="8" fontFamily="serif">N</text>
      <line x1="196" y1="25" x2="196" y2="40" stroke={`${ac}44`} strokeWidth="1"/>
      <text x="110" y="196" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">KÖNIGSMUND</text>
    </g>
  ),
};
...
function GenericCastlePlan({castle,ac,sel,onZone}){
  ...
  return(
    <g>
      ...
      {/* Castle name */}
      <text x="110" y="193" textAnchor="middle" fill={`${ac}44`} fontSize="5.5" fontFamily="serif" letterSpacing="1.5">
    ...
    <div>
      <div style={{position:"relative",
        ...
        <svg viewBox="0 0 220 200" style={{width:"100%",display:"block",cursor:"default"}}>
          ...
          {!selZone&&<text x="110" y="6" textAnchor="middle" fill={`${ac}44`} fontSize="4.5" fontFamily="serif">
        ...
        <div style={{marginTop:"10px",padding:"12px 14px",
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"6px"}}>
            <div>
              ...
              {selZ.l.includes("⚠")&&<div style={{fontSize:"9px",color:"#cc4422",marginTop:"1px",letterSpacing:"1px"}}>⚠ SCHWACHSTELLE</div>}
            </div>
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"2px"}}>
              <div style={{fontSize:"8px",color:"#2a1a0a",letterSpacing:"1px"}}>RÜSTUNG</div>
          ...
          <div style={{fontSize:"10px",color:"#6a5a3a",lineHeight:1.8}}>{selZ.d}</div>
          <button onClick={()=>setSelZone(null)}
            style={{marginTop:"7px",fontSize:"8px",color:"#3a2a14",background:"transparent",
      ...
      <div style={{marginTop:"8px",display:"flex",flexWrap:"wrap",gap:"5px"}}>
        {castle.zones.map(z=>(
          <button key={z.id} onClick={()=>setSelZone(selZone===z.id?null:z.id)}
            ...
            {z.l.includes("⚠")&&<span style={{color:"#cc4422",fontSize:"7px"}}>⚠</span>}
            {z.l.replace(" ⚠","").replace(" ⚠⚠","")}
          </button>
        ))}
      </div>
...
function WorldMap({castles,onSelect,selected}){
  ...
  const [tooltip,setTooltip]=useState(null); // {c, x, y}
  const svgRef=useRef(null);
  const real=castles.filter(c=>COORDS[c.id]&&c.type==="real");
  const fantasy=castles.filter(c=>COORDS[c.id]&&c.type==="fantasy");
  ...
  const handleEnter=(c,e)=>{
    ...
    // Convert SVG coords → pixel coords
    const px=(pos.x/100)*rect.width;
    const py=(pos.y/62)*rect.height;
    setHov(c.id);
    setTooltip({c,px,py});
  };
  const handleLeave=()=>{setHov(null);setTooltip(null);};

  const MapPin=({c,pos,shape="circle"})=>{
    const iS=selected?.id===c.id,iH=hov===c.id;
    const ac=c.theme.accent;
    return(
      <g onClick={()=>onSelect(c)} onMouseEnter={e=>handleEnter(c,e)} onMouseLeave={handleLeave} style={{cursor:"pointer"}}>
        {/* Glow ring for selected/hover */}
        {(iS||iH)&&<circle cx={pos.x} cy={pos.y} r={iS?6:4.5}
          fill="none" stroke={ac} strokeWidth="0.4" strokeOpacity={iS?"0.5":"0.3"}
          style={{animation:"pulse 2s ease infinite"}}/>}
        {/* Outer glow blob */}
        <circle cx={pos.x} cy={pos.y} r={iS?4:iH?3.2:2.4}
          fill={ac} fillOpacity={iS?0.18:iH?0.14:0.07}/>
        {shape==="circle"
          ? <circle cx={pos.x} cy={pos.y} r={iS?2.8:iH?2.2:1.6}
              fill={iS?ac:`${ac}dd`} fillOpacity={iS?1:iH?0.9:0.65}
              stroke={iS?"rgba(255,255,255,0.8)":ac} strokeWidth={iS?0.5:0.25}
              style={{transition:"all .2s ease"}}/>
          : <polygon
              points={`${pos.x},${pos.y-(iS?3.2:iH?2.6:1.9)} ${pos.x+(iS?2.8:iH?2.2:1.6)},${pos.y+(iS?1.6:iH?1.3:1)} ${pos.x-(iS?2.8:iH?2.2:1.6)},${pos.y+(iS?1.6:iH?1.3:1)}`}
              fill={iS?ac:`${ac}dd`} fillOpacity={iS?1:iH?0.9:0.65}
              stroke={iS?"rgba(255,255,255,0.8)":ac} strokeWidth={iS?0.5:0.25}
              style={{transition:"all .2s ease"}}/>
        }
        {/* Castle name label — always visible for selected */}
        {iS&&<>
          <rect x={pos.x-8} y={pos.y-8.5} width="16" height="4.5" rx="0.8"
            fill="rgba(6,4,2,0.88)" stroke={ac} strokeWidth="0.3" strokeOpacity="0.6"/>
          <text x={pos.x} y={pos.y-5.5} textAnchor="middle" dominantBaseline="middle"
            fill={ac} fontSize="2.4" fontFamily="'Palatino Linotype',Georgia,serif" letterSpacing="0.1">
            {c.name.length>12?c.name.slice(0,11)+"…":c.name}
          </text>
        </>}
        {/* Hover name — smaller */}
        {iH&&!iS&&<>
          <rect x={pos.x-7} y={pos.y-7.5} width="14" height="4" rx="0.7"
            fill="rgba(6,4,2,0.82)" stroke={ac} strokeWidth="0.25" strokeOpacity="0.5"/>
          <text x={pos.x} y={pos.y-4.8} textAnchor="middle" dominantBaseline="middle"
            fill="rgba(240,220,180,0.9)" fontSize="2.2" fontFamily="'Palatino Linotype',Georgia,serif">
            {c.name.length>12?c.name.slice(0,11)+"…":c.name}
          </text>
        </>}
      </g>
  ...
  return(
    <div style={{padding:"18px 20px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:"14px"}}>
        <div>
          <div style={{fontSize:"17px",fontWeight:"bold",color:"#f0e6cc",letterSpacing:"0.5px",marginBottom:"3px"}}>🌍 Weltkarte der Festungen</div>
          <div style={{fontSize:"9px",color:"#3a2a14",letterSpacing:"1px"}}>● HISTORISCH  ▲ FANTASY-REICH  · Hover für Details · Klicken zum Analysieren</div>
        </div>
        <div style={{textAlign:"right",fontSize:"9px",color:"#3a2a14"}}>
          <div>{real.length} historische Burgen</div>
          <div>{fantasy.length} Fantasy-Festungen</div>
        </div>
      </div>

      <div style={{position:"relative",borderRadius:"8px",overflow:"hidden",
        boxShadow:"0 0 0 1px rgba(201,168,76,0.12), 0 4px 32px rgba(0,0,0,0.6), inset 0 0 60px rgba(0,0,0,0.4)"}}>
        <svg ref={svgRef} viewBox="0 0 100 62" style={{width:"100%",display:"block"}}>
          <defs>
            <radialGradient id="oceanGrad" cx="55%" cy="45%" r="60%">
              <stop offset="0%" stopColor="#0d1e38"/>
              <stop offset="60%" stopColor="#081428"/>
              <stop offset="100%" stopColor="#040c18"/>
            </radialGradient>
            <radialGradient id="landEurope" cx="50%" cy="50%" r="50%">
              <stop offset="0%" stopColor="#4a6335"/>
              <stop offset="100%" stopColor="#354828"/>
            </radialGradient>
            <radialGradient id="landAsia" cx="50%" cy="50%" r="50%">
              <stop offset="0%" stopColor="#4a6030"/>
              <stop offset="100%" stopColor="#304020"/>
            </radialGradient>
            <radialGradient id="landAfrica" cx="50%" cy="50%" r="50%">
              <stop offset="0%" stopColor="#6a5828"/>
              <stop offset="100%" stopColor="#4a3e1a"/>
            </radialGradient>
            <filter id="landShadow">
              <feDropShadow dx="0" dy="0.4" stdDeviation="0.6" floodColor="rgba(0,0,0,0.5)"/>
            </filter>
            <filter id="pinGlow">
              <feGaussianBlur stdDeviation="1" result="blur"/>
              <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
            </filter>
            <pattern id="oceanGrid" width="5" height="5" patternUnits="userSpaceOnUse">
              <path d="M 5 0 L 0 0 0 5" fill="none" stroke="rgba(255,255,255,0.025)" strokeWidth="0.2"/>
            </pattern>
          </defs>

          {/* Deep ocean */}
          <rect width="100" height="62" fill="url(#oceanGrad)"/>
          {/* Ocean grid pattern */}
          <rect width="100" height="62" fill="url(#oceanGrid)"/>
          {/* Ocean depth lines */}
          {[15,25,35,45,55].map(y=>(
            <line key={y} x1="0" y1={y} x2="100" y2={y}
              stroke="rgba(100,160,255,0.025)" strokeWidth="0.15"/>
          ))}

          {/* ── CONTINENTS (much more detailed paths) ── */}

          {/* EUROPE — detailed */}
          <path filter="url(#landShadow)"
            d="M39 16 L41 14 L43 13.5 L44 12 L45.5 12 L46 11 L47 11.5 L48 11 L49 12 L50 11.5 L51 12 L52 13 L54 13 L56 14 L58 14.5 L59 16 L59 18 L58 19 L57 21 L56 22 L54 23.5 L52 24.5 L51 26 L50 27 L49 27.5 L47 27 L45.5 26.5 L44 26 L42 25 L40 24 L39 22 L38.5 20 L39 18Z"
            fill="url(#landEurope)" stroke="rgba(80,110,55,0.7)" strokeWidth="0.3"/>
          {/* Iberian Peninsula */}
          <path d="M40 24 L42 24 L43.5 25.5 L43 28 L41 29 L39.5 28 L39 26 L39.5 25Z"
            fill="url(#landEurope)" stroke="rgba(80,110,55,0.5)" strokeWidth="0.25"/>
          {/* Italian boot hint */}
          <path d="M49 26 L50 27.5 L51 29.5 L50.5 31 L49.5 31 L49 30 L48.5 28 L48.5 27Z"
            fill="url(#landEurope)" stroke="rgba(80,110,55,0.5)" strokeWidth="0.22"/>
          {/* Scandinavian peninsula */}
          <path d="M47 11.5 L48.5 11 L49.5 10 L50 9 L50.5 8.5 L51 9 L51.5 10 L52 12 L51 12Z"
            fill="url(#landEurope)" stroke="rgba(80,110,55,0.5)" strokeWidth="0.22"/>
          {/* British Isles */}
          <ellipse cx="42.5" cy="15.5" rx="1.4" ry="2.2" fill="url(#landEurope)" stroke="rgba(80,110,55,0.6)" strokeWidth="0.25"/>
          <ellipse cx="41" cy="14.8" rx="0.8" ry="1.4" fill="url(#landEurope)" stroke="rgba(80,110,55,0.5)" strokeWidth="0.2"/>

          {/* NORTH AFRICA */}
          <path filter="url(#landShadow)"
            d="M39 29 L41.5 27 L44 26.5 L48 26.5 L52 27 L56 27 L60 27.5 L63 28 L65 29 L66 31 L66 35 L65 38 L62 40 L58 41 L54 41.5 L50 41.5 L46 41 L43 40 L40 38.5 L38.5 36 L38 33 L38 31Z"
            fill="url(#landAfrica)" stroke="rgba(110,90,35,0.6)" strokeWidth="0.3"/>
          {/* Mediterranean cutout */}
          <path d="M44 26.5 L58 26.5 L59.5 28.5 L58 30.5 L52 31 L45 31 L43.5 29 Z"
            fill="url(#oceanGrad)" opacity="0.95"/>

          {/* MIDDLE EAST */}
          <path filter="url(#landShadow)"
            d="M58 27 L62 26.5 L66 26.5 L70 27 L72 28 L73 30 L72 33 L70 36 L68 38 L65 39 L63 38 L60 37 L58 35 L57 33 L57 30Z"
            fill="url(#landAfrica)" stroke="rgba(110,90,35,0.55)" strokeWidth="0.28"/>

          {/* SUB-SAHARAN AFRICA hint */}
          <path d="M42 41 L50 41.5 L56 41 L60 41 L63 42 L64 44 L63 47 L60 49.5 L56 51 L50 52 L44 51 L40 49 L38.5 46 L38.5 43 L40 41Z"
            fill="url(#landAfrica)" stroke="rgba(110,90,35,0.45)" strokeWidth="0.28" opacity="0.8"/>

          {/* ASIA — large landmass */}
          <path filter="url(#landShadow)"
            d="M58 14 L62 13 L66 13 L70 13 L75 12.5 L80 13 L85 13 L90 14 L93 16 L94 19 L93 22 L91 25 L88 27.5 L84 29.5 L80 30.5 L76 31 L72 30.5 L68 29 L65 27.5 L62 26.5 L59 25 L58 22 L57.5 20 L57.5 18Z"
            fill="url(#landAsia)" stroke="rgba(70,95,48,0.6)" strokeWidth="0.3"/>
          {/* India subcontinent */}
          <path d="M68 29 L70 30 L72 32 L73 34 L72 37 L70 39.5 L68 41 L66 40 L64 38 L63 36 L64 33 L66 31 L68 30Z"
            fill="url(#landAsia)" stroke="rgba(70,95,48,0.5)" strokeWidth="0.25"/>
          {/* Southeast Asia hint */}
          <path d="M78 31 L80 32 L82 33 L83 35 L82 37 L80 37.5 L78 37 L77 35 L77 33Z"
            fill="url(#landAsia)" stroke="rgba(70,95,48,0.45)" strokeWidth="0.22" opacity="0.85"/>

          {/* JAPAN */}
          <ellipse cx="83.5" cy="27" rx="1.2" ry="3.8"
            fill="url(#landAsia)" stroke="rgba(70,95,48,0.6)" strokeWidth="0.25"
            transform="rotate(-20 83.5 27)"/>

          {/* CENTRAL ASIA / PERSIA region highlight — lighter tone */}
          <path d="M64 23 L70 22.5 L74 23 L76 25 L74 27.5 L70 28.5 L66 28 L63 26.5Z"
            fill="rgba(90,75,38,0.3)" stroke="rgba(120,95,45,0.3)" strokeWidth="0.2"/>

          {/* Fantasy continent region — top left, visually distinct */}
          <rect x="1" y="1" width="36" height="28" rx="2"
            fill="rgba(20,10,35,0.7)" stroke="rgba(80,50,130,0.3)" strokeWidth="0.3" strokeDasharray="1,0.8"/>
          {/* Fantasy land texture */}
          <path d="M3 8 L8 6 L14 6.5 L18 5.5 L22 6 L26 5.5 L30 6.5 L34 6 L36 8 L36 16 L34 18 L30 19 L26 20 L22 21.5 L18 21 L14 22 L10 21.5 L6 20 L3 18 L2 14 L2 10Z"
            fill="rgba(40,25,65,0.5)" stroke="rgba(90,60,150,0.35)" strokeWidth="0.3"/>
          {/* Westeros region */}
          <path d="M3 20 L8 20.5 L12 21 L14 22 L13 25 L10 26.5 L6 26.5 L3 25 L2.5 23Z"
            fill="rgba(30,35,55,0.5)" stroke="rgba(70,90,140,0.35)" strokeWidth="0.3"/>
          {/* Fantasy region labels */}
          <text x="19" y="4" textAnchor="middle" fill="rgba(150,100,220,0.45)" fontSize="2.1"
            fontFamily="'Palatino Linotype',Georgia,serif" letterSpacing="0.3">✦ MITTELERDE</text>
          <text x="8" y="28.5" textAnchor="middle" fill="rgba(100,130,200,0.4)" fontSize="1.8"
            fontFamily="'Palatino Linotype',Georgia,serif">❄ WESTEROS</text>
          {/* Divider line between fantasy regions */}
          <line x1="1" y1="20" x2="36" y2="20" stroke="rgba(80,80,160,0.18)" strokeWidth="0.2" strokeDasharray="0.8,0.8"/>

          {/* Ocean latitude lines (subtle) */}
          <line x1="0" y1="31" x2="100" y2="31" stroke="rgba(100,160,255,0.04)" strokeWidth="0.3"/>

          {/* ── CASTLE PINS ── */}
          <g filter="url(#pinGlow)">
            {real.map(c=>{const pos=COORDS[c.id];return pos?<MapPin key={c.id} c={c} pos={pos} shape="circle"/>:null;})}
            {fantasy.map(c=>{const pos=COORDS[c.id];return pos?<MapPin key={c.id} c={c} pos={pos} shape="triangle"/>:null;})}
          </g>
        </svg>

        {/* Floating tooltip */}
        {hC&&<div style={{position:"absolute",bottom:"10px",left:"12px",right:"12px",
          padding:"10px 14px",
          background:"linear-gradient(135deg,rgba(10,7,3,0.97) 0%,rgba(18,12,5,0.97) 100%)",
          border:`1px solid ${hC.theme.accent}55`,
          borderLeft:`3px solid ${hC.theme.accent}`,
          borderRadius:"6px",display:"flex",gap:"12px",alignItems:"center",
          pointerEvents:"none",
          boxShadow:`0 4px 20px rgba(0,0,0,0.7), 0 0 12px ${hC.theme.accent}18`}}>
          <div style={{fontSize:"22px",flexShrink:0}}>{hC.icon}</div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:"12px",fontWeight:"bold",color:"#f0e8d0",marginBottom:"2px"}}>{hC.name}</div>
            <div style={{fontSize:"9px",color:"#5a4a28",letterSpacing:"0.5px"}}>{hC.sub}</div>
            <div style={{fontSize:"8px",color:"#3a2a14",marginTop:"2px"}}>{hC.loc} · {hC.era}</div>
          </div>
          <div style={{textAlign:"center",flexShrink:0}}>
            <div style={{fontSize:"20px",fontWeight:"bold",color:rCol(avg(hC)),lineHeight:1}}>{avg(hC)}</div>
            <div style={{fontSize:"7px",color:"#2a1a08",letterSpacing:"1px",marginTop:"1px"}}>SCORE</div>
          </div>
          <div style={{fontSize:"8px",color:hC.theme.accent,flexShrink:0,letterSpacing:"1px"}}>KLICKEN →</div>
        </div>}
      </div>

      {/* Legend */}
      <div style={{display:"flex",gap:"18px",marginTop:"10px",padding:"8px 12px",background:"rgba(0,0,0,0.25)",borderRadius:"4px",border:"1px solid rgba(255,255,255,0.04)"}}>
        <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
          <svg width="12" height="12" viewBox="0 0 12 12"><circle cx="6" cy="6" r="4" fill="rgba(201,168,76,0.7)"/></svg>
          <span style={{fontSize:"9px",color:"#4a3a20"}}>Historische Burg</span>
        </div>
        <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
          <svg width="12" height="12" viewBox="0 0 12 12"><polygon points="6,1 11,10 1,10" fill="rgba(136,102,204,0.7)"/></svg>
          <span style={{fontSize:"9px",color:"#4a3a20"}}>Fantasy-Festung</span>
        </div>
        <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
          <svg width="12" height="12" viewBox="0 0 12 12"><circle cx="6" cy="6" r="4" fill="rgba(201,168,76,0.25)" stroke="rgba(201,168,76,0.9)" strokeWidth="1.5"/></svg>
          <span style={{fontSize:"9px",color:"#4a3a20"}}>Ausgewählt</span>
        </div>
        <div style={{marginLeft:"auto",fontSize:"9px",color:"#2a1a08"}}>{real.length + fantasy.length} Festungen kartiert</div>
      </div>
    </div>
  );
}
    ...
    return(
      <button onClick={()=>onSelect(c)} onMouseEnter={()=>setHov(c.id)} onMouseLeave={()=>setHov(null)}
        ...
        <div style={{
          ...
          {/* Type badge */}
          <div style={{position:"absolute",top:"8px",right:"8px",padding:"1px 5px",borderRadius:"2px",fontSize:"6px",letterSpacing:"1px",
          ...
          {hs&&<div style={{position:"absolute",top:"7px",left:"8px",fontSize:"9px",filter:"drop-shadow(0 1px 3px rgba(0,0,0,0.6))"}}>{hs.won?"✅":"❌"}</div>}
            ...
            <div style={{display:"flex",alignItems:"flex-start",gap:"8px",marginBottom:"9px"}}>
              ...
              <div style={{flexShrink:0,position:"relative",width:"36px",height:"36px"}}>
                ...
                <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",
                  fontSize:"10px",fontWeight:"bold",color:rCol(sc),fontFamily:"monospace"}}>{sc}</div>
              </div>
            </div>
            {/* Name */}
            <div style={{fontSize:"11px",fontWeight:"bold",color:isH?"#f5edd8":"#c0b090",lineHeight:1.25,marginBottom:"2px",letterSpacing:"0.2px"}}>{c.name}</div>
            <div style={{fontSize:"8px",color:isH?c.theme.accent:"#3a2a14",marginBottom:"1px",letterSpacing:"0.3px"}}>{c.sub}</div>
            <div style={{fontSize:"7px",color:"#2a1a08",marginBottom:"9px"}}>{c.loc} · {c.era}</div>
            {/* Stat bars */}
              ...
              {cats.map((k,i)=>{
                ...
                return(
                  <div key={k} style={{display:"flex",alignItems:"center",gap:"5px"}}>
                    <div style={{fontSize:"6px",color:"#2a1a08",width:"28px",flexShrink:0,letterSpacing:"0.3px"}}>{catLabels[i]}</div>
                    ...
                    <div style={{fontSize:"7px",color:isMax?c.theme.accent:isMin?"#884422":rCol(v),fontFamily:"monospace",width:"18px",textAlign:"right",flexShrink:0}}>{v}</div>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{padding:"5px 12px",borderTop:"1px solid rgba(255,255,255,0.04)",background:"rgba(0,0,0,0.2)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:"7px",color:isH?c.theme.accent:"#2a1a08",letterSpacing:"1px"}}>{c.epoch.toUpperCase()}</span>
            {hs?.rating&&<span style={{fontSize:"7px",color:rCol(hs.rating*10),fontFamily:"monospace"}}>{hs.rating}/10</span>}
          </div>
        </div>
      </button>
    );
    ...
    <div style={{padding:"18px 20px"}}>
      ...
      <div style={{display:"flex",gap:"7px",flexWrap:"wrap",marginBottom:"16px",alignItems:"center",padding:"10px 14px",background:"rgba(0,0,0,0.3)",border:"1px solid rgba(255,255,255,0.05)",borderRadius:"6px"}}>
        ...
        {[{k:"all",l:"Alle"},{k:"real",l:"⚜ Historisch"},{k:"fantasy",l:"✦ Fantasy"}].map(f=>(
          <button key={f.k} onClick={()=>setFilter(f.k)} style={{padding:"3px 9px",fontSize:"8px",letterSpacing:"1px",background:filter===f.k?"rgba(201,168,76,0.12)":"transparent",border:`1px solid ${filter===f.k?"rgba(201,168,76,0.35)":"rgba(255,255,255,0.07)"}`,color:filter===f.k?"#c9a84c":"#3a2a10",borderRadius:"3px",cursor:"pointer",transition:"all .14s"}}>{f.l}</button>
        ))}
        <div style={{height:"16px",width:"1px",background:"rgba(255,255,255,0.08)",margin:"0 1px"}}/>
        <select value={epochFilter} onChange={e=>setEpochFilter(e.target.value)} style={{padding:"3px 7px",background:"rgba(0,0,0,0.5)",border:"1px solid rgba(255,255,255,0.08)",color:"#6a5a30",fontSize:"8px",borderRadius:"3px",outline:"none",fontFamily:"inherit"}}>
          <option value="">Alle Epochen</option>{epochs.map(e=><option key={e} value={e}>{e}</option>)}
        </select>
        <select value={regionFilter} onChange={e=>setRegionFilter(e.target.value)} style={{padding:"3px 7px",background:"rgba(0,0,0,0.5)",border:"1px solid rgba(255,255,255,0.08)",color:"#6a5a30",fontSize:"8px",borderRadius:"3px",outline:"none",fontFamily:"inherit"}}>
        ...
        {[{k:"default",l:"Gruppiert"},{k:"score",l:"↓ Score"},{k:"epoch",l:"Chronol."},{k:"name",l:"A–Z"}].map(s=>(
          <button key={s.k} onClick={()=>setSortBy(s.k)} style={{padding:"3px 7px",fontSize:"7px",letterSpacing:"1px",background:sortBy===s.k?"rgba(201,168,76,0.08)":"transparent",border:`1px solid ${sortBy===s.k?"rgba(201,168,76,0.25)":"rgba(255,255,255,0.05)"}`,color:sortBy===s.k?"#c9a84c":"#2a1a08",borderRadius:"3px",cursor:"pointer"}}>{s.l}</button>
        ))}
        <div style={{marginLeft:"auto",display:"flex",gap:"7px",alignItems:"center"}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Burg suchen…" style={{padding:"4px 8px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",color:"#a09070",fontSize:"8px",borderRadius:"3px",outline:"none",width:"95px",fontFamily:"inherit"}}/>
          <span style={{fontSize:"8px",color:"#2a1a08",fontFamily:"monospace"}}>{filtered.length}/{castles.length}</span>
        </div>
      </div>
        ...
        ? Object.entries(grouped).sort(([a],[b])=>{const o=["europa","nahost","ostasien","suedostasien","mittelerde","westeros"];return o.indexOf(a)-o.indexOf(b);}).map(([region,cards])=>(
          <div key={region} style={{marginBottom:"22px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"10px",paddingBottom:"6px",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
              ...
              <span style={{fontSize:"8px",color:"#2a1a08"}}>{cards.length} Festungen</span>
...
function RoleplaySiege({castle,onScore,general,season}){
  ...
  const send=async()=>{
    ...
    try{
      const res=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,system:`Du spielst ${castle.defender||"den Burgherrn"} der ${castle.name} verteidigt (${castle.era}).${genBonus}${seasonPen}
    ...
    <div>
      <div style={{display:"flex",gap:"6px",marginBottom:"10px"}}>
        <div style={{flex:1,padding:"9px 12px",background:"rgba(130,35,8,.1)",border:"1px solid rgba(130,35,8,.22)",borderRadius:"4px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{fontSize:"8px",color:"#cc5533",letterSpacing:"2px",marginBottom:"1px"}}>🎭 BELAGERUNG</div><div style={{fontSize:"10px",color:"#7a5040"}}>{castle.defender||"Burgherr"}{general&&` vs. ${general.name}`}</div></div>
          <div style={{textAlign:"right"}}><div style={{fontSize:"18px",fontWeight:"bold",color:"#c9a84c"}}>{turn}</div><div style={{fontSize:"7px",color:"#2a1a08"}}>ZUG</div></div>
        </div>
        <button onClick={()=>setShowJournal(j=>!j)} style={{padding:"9px 11px",background:showJournal?"rgba(201,168,76,0.1)":"rgba(255,255,255,0.02)",border:`1px solid ${showJournal?"rgba(201,168,76,0.25)":"rgba(255,255,255,0.05)"}`,color:showJournal?"#c9a84c":"#3a2a14",borderRadius:"4px",cursor:"pointer",fontSize:"10px"}}>
          📖 {journal.length}
        </button>
        {journal.length>0&&<button onClick={exportLog} style={{padding:"9px 11px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.05)",color:"#3a2a14",borderRadius:"4px",cursor:"pointer",fontSize:"10px"}}>⬇️</button>}
      </div>
      ...
      {ev&&<div style={{padding:"9px 12px",marginBottom:"9px",background:"rgba(170,110,15,0.13)",border:"1px solid rgba(170,110,15,.3)",borderRadius:"4px",animation:"fadeIn .2s ease",display:"flex",gap:"8px",alignItems:"center"}}>
        ...
        <div><div style={{fontSize:"8px",letterSpacing:"2px",color:"#c9a84c",marginBottom:"1px"}}>{ev.t}</div><div style={{fontSize:"9px",color:"#7a6038"}}>{ev.txt}</div></div>
      </div>}
      ...
      {showJournal&&journal.length>0&&<div style={{marginBottom:"9px",padding:"10px",background:"rgba(0,0,0,0.35)",border:"1px solid rgba(201,168,76,0.1)",borderRadius:"4px",maxHeight:"160px",overflowY:"auto"}}>
        {journal.map((e,i)=><div key={i} style={{padding:"3px 0",borderBottom:"1px solid rgba(255,255,255,0.02)",fontSize:"9px",color:"#5a4a30",display:"flex",gap:"6px"}}>
          <span style={{color:"#2a1a0a",flexShrink:0}}>#{e.turn}</span><span style={{flex:1}}>{e.action}</span>
      ...
      {outcome&&<div style={{padding:"13px",marginBottom:"9px",textAlign:"center",background:outcome==="v"?"rgba(25,75,15,.18)":"rgba(90,15,8,.18)",border:`1px solid ${outcome==="v"?"rgba(40,110,20,.32)":"rgba(120,20,10,.32)"}`,borderRadius:"4px"}}>
        ...
        <div style={{fontSize:"9px",color:"#3a2a14",marginTop:"2px"}}>{turn} Züge · {castle.name}</div>
        <button onClick={reset} style={{marginTop:"8px",padding:"5px 13px",background:"rgba(201,168,76,0.09)",border:"1px solid rgba(201,168,76,0.22)",color:"#c9a84c",borderRadius:"3px",cursor:"pointer",fontSize:"9px"}}>Neu starten</button>
      </div>}
      ...
      <div style={{height:"310px",overflowY:"auto",marginBottom:"9px"}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:"flex",gap:"6px",marginBottom:"8px",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            {m.role==="assistant"&&<div style={{width:"20px",height:"20px",borderRadius:"50%",flexShrink:0,background:"rgba(130,35,8,.2)",border:"1px solid rgba(130,35,8,.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"10px",marginTop:"2px"}}>🛡</div>}
            <div style={{maxWidth:"88%",padding:"6px 10px",background:m.role==="user"?"rgba(201,168,76,0.06)":m.type==="sys"?"rgba(90,45,8,0.14)":"rgba(255,255,255,0.02)",border:`1px solid ${m.role==="user"?"rgba(201,168,76,0.13)":m.type==="sys"?"rgba(130,60,12,.2)":"rgba(255,255,255,0.04)"}`,borderRadius:m.role==="user"?"9px 9px 3px 9px":"9px 9px 9px 3px",fontSize:"10px",color:m.role==="user"?"#e8d8b0":"#7a6a50",lineHeight:1.75}}>
              {m.text.split("**").map((p,j)=>j%2===1?<strong key={j} style={{color:m.role==="user"?"#c9a84c":"#b89860"}}>{p}</strong>:<span key={j}>{p}</span>)}
            </div>
          </div>
        ))}
        {loading&&<div style={{display:"flex",gap:"6px"}}><div style={{width:"20px",height:"20px",borderRadius:"50%",background:"rgba(130,35,8,.2)",border:"1px solid rgba(130,35,8,.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"10px"}}>🛡</div><div style={{padding:"6px 10px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.04)",borderRadius:"9px 9px 9px 3px",display:"flex",gap:"3px",alignItems:"center"}}>{[0,1,2].map(i=><div key={i} style={{width:"4px",height:"4px",borderRadius:"50%",background:"#c9a84c",animation:`bounce 1.2s ease infinite`,animationDelay:`${i*.2}s`}}/>)}</div></div>}
        <div ref={bot}/>
      </div>

      {!outcome&&<>
        <div style={{display:"flex",gap:"3px",flexWrap:"wrap",marginBottom:"6px"}}>{qas.map((q,i)=><button key={i} onClick={()=>setInput(q)} style={{fontSize:"8px",padding:"2px 7px",background:"rgba(201,168,76,0.03)",border:"1px solid rgba(201,168,76,0.09)",color:"#3a2a16",borderRadius:"8px",cursor:"pointer"}}>{q.slice(0,32)}…</button>)}</div>
        <div style={{display:"flex",gap:"5px"}}>
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Dein Angriffszug…" style={{flex:1,padding:"7px 10px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(201,168,76,0.13)",borderRadius:"4px",color:"#e8d8b0",fontSize:"10px",outline:"none",fontFamily:"inherit"}}/>
...
function SiegeSimulator({castle,onScore,general,season}){
  ...
  return(
    <div>
      <div style={{padding:"9px 12px",background:"rgba(130,60,8,.07)",border:"1px solid rgba(130,60,8,.16)",borderRadius:"4px",marginBottom:"12px",fontSize:"10px",color:"#7a6038",lineHeight:1.7}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px"}}>
        <span style={{fontSize:"7px",color:"#2a1a0a",letterSpacing:"2px"}}>RESSOURCEN VERTEILEN</span>
        <span style={{fontSize:"11px",fontWeight:"bold",color:used===TOTAL_RES?"#8aaa68":"#c9a84c"}}>{used}/{TOTAL_RES}</span>
      </div>
          ...
          return(
            <div key={k} style={{padding:"7px 9px",background:"rgba(255,255,255,.016)",border:`1px solid ${alloc[k]>0?`${r.c}28`:"rgba(255,255,255,.03)"}`,borderRadius:"3px"}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:"4px"}}>
                <div style={{fontSize:"8px",color:alloc[k]>0?r.c:"#2a1a14"}}>{r.i} {r.l}</div>
                <div style={{display:"flex",gap:"3px",alignItems:"center"}}>
                  {genBonus!==0&&<span style={{fontSize:"7px",color:genBonus>0?"#8aaa68":"#cc5544"}}>{genBonus>0?"+":""}{genBonus}</span>}
                  {seasonBonus>0&&<span style={{fontSize:"7px",color:"#88cc44"}}>+{seasonBonus}</span>}
                  {seasonPen<0&&<span style={{fontSize:"7px",color:"#cc4433"}}>{seasonPen}</span>}
                  <div style={{fontSize:"11px",fontWeight:"bold",color:alloc[k]>0?r.c:"#1a0a00"}}>{alloc[k]}</div>
                </div>
              </div>
      ...
      {result&&<div style={{marginTop:"12px",padding:"12px",background:result.success?"rgba(22,70,12,.1)":"rgba(85,12,7,.1)",border:`1px solid ${result.success?"rgba(35,95,18,.22)":"rgba(105,18,10,.22)"}`,borderRadius:"4px"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:"8px"}}>
          <div><div style={{fontSize:"7px",letterSpacing:"2px",color:result.success?"#5aaa42":"#cc3322",marginBottom:"2px"}}>{result.success?"✅ ERFOLG":"❌ GESCHEITERT"}</div><div style={{fontSize:"12px",fontWeight:"bold",color:"#f0e0c0"}}>{result.title}</div>{result.daysElapsed&&<div style={{fontSize:"8px",color:"#3a2a14",marginTop:"1px"}}>~{result.daysElapsed} Tage</div>}</div>
          <div style={{textAlign:"center"}}><div style={{fontSize:"22px",fontWeight:"bold",color:rCol(result.rating*10)}}>{result.rating}</div><div style={{fontSize:"7px",color:"#2a1a0a"}}>/10</div></div>
        </div>
        <div style={{fontSize:"10px",color:"#6a5038",lineHeight:1.75,marginBottom:"8px"}}>{result.outcome}</div>
        {result.keyMoment&&<div style={{padding:"7px 9px",background:"rgba(0,0,0,.22)",borderLeft:"3px solid rgba(201,168,76,.28)",borderRadius:"2px",fontSize:"9px",color:"#5a4030",lineHeight:1.8,marginBottom:"8px"}}>{result.keyMoment}</div>}
        {result.phases?.length>0&&<div style={{marginBottom:"8px"}}>{result.phases.map((p,i)=><div key={i} style={{fontSize:"9px",color:"#4a3a22",padding:"2px 0",borderBottom:"1px solid rgba(255,255,255,.02)",display:"flex",gap:"5px"}}><span style={{color:"#c9a84c",flexShrink:0}}>{i+1}.</span>{p}</div>)}</div>}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"7px",marginBottom:"7px"}}>
          {result.whatWorked?.length>0&&<div><div style={{fontSize:"7px",color:"#3a7022",letterSpacing:"2px",marginBottom:"3px"}}>FUNKTIONIERTE</div>{result.whatWorked.map((w,i)=><div key={i} style={{fontSize:"8px",color:"#2a5518",padding:"1px 0"}}>✓ {w}</div>)}</div>}
          {result.mistakes?.length>0&&<div><div style={{fontSize:"7px",color:"#702a18",letterSpacing:"2px",marginBottom:"3px"}}>FEHLER</div>{result.mistakes.map((m,i)=><div key={i} style={{fontSize:"8px",color:"#5a2010",padding:"1px 0"}}>✗ {m}</div>)}</div>}
        </div>
        {result.generalBonus&&<div style={{fontSize:"8px",color:"#4a4030",fontStyle:"italic",borderTop:"1px solid rgba(255,255,255,0.03)",paddingTop:"5px",marginBottom:"3px"}}>⚔️ {result.generalBonus}</div>}
        {result.seasonEffect&&<div style={{fontSize:"8px",color:"#3a4030",fontStyle:"italic"}}>🌿 {result.seasonEffect}</div>}
        {result.historicalParallel&&<div style={{fontSize:"8px",color:"#2a1a0a",fontStyle:"italic",marginTop:"4px"}}>📜 {result.historicalParallel}</div>}
      </div>}
    </div>
  );
}
...
function WhatIf({castle}){
  ...
  const analyze=async()=>{
    ...
    try{
      const r=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,messages:[{role:"user",content:`Kontrafaktischer Militärhistoriker.\nBURG: ${castle.name} (${castle.era}). HISTORISCH: ${castle.history}. STÄRKEN: ${castle.strengths.join("; ")}. SCHWÄCHEN: ${castle.weaknesses.join("; ")}.\n\nSZENARIO: ${scen}\n\nNUR JSON:\n{"likelihood":1-10,"outcome":"1 Satz","analysis":"3-4 Sätze kontrafaktische Analyse (Deutsch)","wouldHaveFallen":true/false,"keyFactor":"1 Satz","timeChange":"wäre früher/später gefallen: 1 Satz","historicalParallels":["Beispiel 1","Beispiel 2"],"confidence":"hoch/mittel/niedrig"}`}]})});
      const data=await r.json();
      setRes(JSON.parse(data.content?.map(b=>b.text||"").join("").replace(/```json|```/g,"").trim()));
    }catch{setRes({likelihood:5,outcome:"Fehler",analysis:"",wouldHaveFallen:false,keyFactor:"",timeChange:"",historicalParallels:[],confidence:"niedrig"});}
    setLoading(false);
  };
  return(
    <div>
      <div style={{padding:"9px 12px",background:"rgba(50,60,100,.08)",border:"1px solid rgba(70,80,140,.2)",borderRadius:"4px",marginBottom:"12px",fontSize:"10px",color:"#7a8ab0",lineHeight:1.7}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:"3px",marginBottom:"9px"}}>
        {presets.map((p,i)=><button key={i} onClick={()=>setScen(p)} style={{fontSize:"8px",padding:"3px 8px",background:scen===p?"rgba(136,153,204,.12)":"rgba(255,255,255,.018)",border:`1px solid ${scen===p?"rgba(136,153,204,.3)":"rgba(255,255,255,.05)"}`,color:scen===p?"#8899cc":"#3a3a5a",borderRadius:"9px",cursor:"pointer"}}>{p.length>52?p.slice(0,50)+"…":p}</button>)}
      </div>
      <div style={{display:"flex",gap:"5px",marginBottom:"10px"}}>
        <input value={scen} onChange={e=>setScen(e.target.value)} onKeyDown={e=>e.key==="Enter"&&analyze()} placeholder="Eigenes Szenario…" style={{flex:1,padding:"7px 10px",background:"rgba(255,255,255,.04)",border:"1px solid rgba(136,153,204,.18)",borderRadius:"4px",color:"#e8d8b0",fontSize:"10px",outline:"none",fontFamily:"inherit"}}/>
        <button onClick={analyze} disabled={loading||!scen.trim()} style={{padding:"7px 12px",background:scen.trim()&&!loading?"rgba(70,85,140,.2)":"rgba(255,255,255,.02)",border:`1px solid ${scen.trim()&&!loading?"rgba(70,85,140,.4)":"rgba(255,255,255,.05)"}`,color:scen.trim()&&!loading?"#8899cc":"#1a1a2a",borderRadius:"4px",cursor:"pointer",fontSize:"10px"}}>{loading?"⏳":"🌀"}</button>
      </div>
      {res&&<div style={{padding:"12px",background:"rgba(0,0,0,.3)",border:"1px solid rgba(136,153,204,.14)",borderRadius:"4px",animation:"fadeIn .25s ease"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}>
          <div><div style={{fontSize:"7px",letterSpacing:"2px",color:res.wouldHaveFallen?"#cc4433":"#5a9a42",marginBottom:"2px"}}>{res.wouldHaveFallen?"💀 WÄRE GEFALLEN":"🛡️ HÄTTE GEHALTEN"}</div><div style={{fontSize:"12px",fontWeight:"bold",color:"#e0d0b0"}}>{res.outcome}</div></div>
          <div style={{textAlign:"center",padding:"5px 9px",background:"rgba(136,153,204,.08)",border:"1px solid rgba(136,153,204,.14)",borderRadius:"3px"}}>
            ...
            <div style={{fontSize:"7px",color:"#2a2a40"}}>PLAUSIB.</div>
          </div>
        </div>
        <div style={{fontSize:"10px",color:"#5a5a80",lineHeight:1.8,marginBottom:"8px",fontStyle:"italic",borderLeft:"3px solid rgba(136,153,204,.22)",paddingLeft:"9px"}}>{res.analysis}</div>
        {res.keyFactor&&<div style={{padding:"6px 9px",background:"rgba(136,153,204,.06)",border:"1px solid rgba(136,153,204,.1)",borderRadius:"3px",marginBottom:"7px",fontSize:"9px",color:"#6a6a8a"}}><strong style={{color:"#8899cc"}}>Entscheidend:</strong> {res.keyFactor}</div>}
        {res.timeChange&&<div style={{fontSize:"8px",color:"#4a4a60",marginBottom:"5px"}}>⏱ {res.timeChange}</div>}
        {res.historicalParallels?.length>0&&<div><div style={{fontSize:"7px",color:"#2a2a48",letterSpacing:"2px",marginBottom:"3px"}}>PARALLELEN</div>{res.historicalParallels.map((p,i)=><div key={i} style={{fontSize:"8px",color:"#3a3a58",padding:"1px 0",display:"flex",gap:"5px"}}><span style={{color:"#8899cc"}}>›</span>{p}</div>)}</div>}
      </div>}
    </div>
  );
}
...
function AIAdvisor({castle}){
  ...
  const send=async()=>{
    ...
    try{
      ...
      const data=await res.json();
      setMsgs(m=>[...m,{role:"assistant",text:data.content?.map(b=>b.text||"").join("")||"Fehler."}]);
    }catch{setMsgs(m=>[...m,{role:"assistant",text:"Verbindungsfehler."}]);}
    setLoading(false);
  };
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"380px"}}>
      <div style={{flex:1,overflowY:"auto",marginBottom:"8px"}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:"flex",gap:"6px",marginBottom:"8px",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            {m.role==="assistant"&&<div style={{width:"20px",height:"20px",borderRadius:"50%",flexShrink:0,background:`${castle.theme.accent}18`,border:`1px solid ${castle.theme.accent}38`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"10px",marginTop:"2px"}}>⚔️</div>}
            <div style={{maxWidth:"87%",padding:"6px 10px",background:m.role==="user"?"rgba(201,168,76,0.06)":"rgba(255,255,255,0.02)",border:`1px solid ${m.role==="user"?"rgba(201,168,76,0.12)":"rgba(255,255,255,0.04)"}`,borderRadius:m.role==="user"?"9px 9px 3px 9px":"9px 9px 9px 3px",fontSize:"10px",color:m.role==="user"?"#e8d8b0":"#7a6a52",lineHeight:1.75}}>
              {m.text.split("**").map((p,j)=>j%2===1?<strong key={j} style={{color:castle.theme.accent}}>{p}</strong>:<span key={j}>{p}</span>)}
            </div>
          </div>
        ))}
        {loading&&<div style={{display:"flex",gap:"6px"}}><div style={{width:"20px",height:"20px",borderRadius:"50%",background:`${castle.theme.accent}18`,border:`1px solid ${castle.theme.accent}38`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"10px"}}>⚔️</div><div style={{padding:"6px 10px",background:"rgba(255,255,255,0.02)",borderRadius:"9px 9px 9px 3px",display:"flex",gap:"3px",alignItems:"center"}}>{[0,1,2].map(i=><div key={i} style={{width:"4px",height:"4px",borderRadius:"50%",background:castle.theme.accent,animation:"bounce 1.2s ease infinite",animationDelay:`${i*.2}s`}}/>)}</div></div>}
        <div ref={bot}/>
      </div>
      <div style={{display:"flex",gap:"3px",flexWrap:"wrap",marginBottom:"5px"}}>{qs.map((q,i)=><button key={i} onClick={()=>setInput(q)} style={{fontSize:"8px",padding:"2px 7px",background:`${castle.theme.accent}07`,border:`1px solid ${castle.theme.accent}16`,color:"#3a2a18",borderRadius:"8px",cursor:"pointer"}}>{q}</button>)}</div>
      <div style={{display:"flex",gap:"5px"}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Frage stellen…" style={{flex:1,padding:"6px 10px",background:"rgba(255,255,255,.04)",border:`1px solid ${castle.theme.accent}1a`,borderRadius:"4px",color:"#e8d8b0",fontSize:"10px",outline:"none",fontFamily:"inherit"}}/>
  ...
  return(
    <div>
      <div style={{marginBottom:"10px",display:"flex",gap:"8px",alignItems:"center"}}>
        ...
        <div style={{padding:"5px 10px",background:"rgba(0,0,0,.28)",border:"1px solid rgba(255,255,255,.05)",borderRadius:"4px",fontSize:"10px",fontWeight:"bold",color:rem<=0?"#8aaa68":"#c9a84c",whiteSpace:"nowrap"}}>💰 {rem}</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"5px",marginBottom:"9px"}}>
        {[{l:"Verteidigung",v:tD},{l:"Versorgung",v:tS},{l:"Position",v:tP}].map(s=>(
          <div key={s.l} style={{textAlign:"center",padding:"7px 4px",background:"rgba(255,255,255,.018)",border:"1px solid rgba(255,255,255,.03)",borderRadius:"4px"}}>
            ...
            <div style={{fontSize:"7px",color:"#1a0e08",letterSpacing:"1px",marginTop:"1px"}}>{s.l.toUpperCase()}</div>
          </div>
        ))}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px",marginBottom:"9px",maxHeight:"300px",overflowY:"auto"}}>
        {BUILDER.map(el=>{const isS=sel.includes(el.id),canA=rem>=(el.cost)||isS;return(
          <button key={el.id} onClick={()=>toggle(el.id)} style={{padding:"7px 9px",textAlign:"left",background:isS?"rgba(201,168,76,0.07)":"rgba(255,255,255,0.016)",border:`1px solid ${isS?"rgba(201,168,76,0.26)":canA?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.016)"}`,borderRadius:"4px",cursor:canA?"pointer":"not-allowed",opacity:canA?1:.3,transition:"all .1s"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:"1px"}}><div style={{fontSize:"9px",color:isS?"#e0c878":"#4a3a28"}}>{el.i} {el.l}</div><div style={{fontSize:"8px",fontWeight:"bold",color:isS?"#c9a84c":"#1a0e08"}}>💰{el.cost}</div></div>
            <div style={{fontSize:"7px",color:"#1a0e0e",lineHeight:1.4}}>{el.desc}</div>
            {isS&&<div style={{display:"flex",gap:"4px",marginTop:"2px"}}>{el.def>0&&<span style={{fontSize:"6px",color:"#cc7744"}}>🛡+{el.def}</span>}{el.sup>0&&<span style={{fontSize:"6px",color:"#4488cc"}}>📦+{el.sup}</span>}{el.pos>0&&<span style={{fontSize:"6px",color:"#88aa44"}}>⛰+{el.pos}</span>}</div>}
          </button>);})}
      </div>
      <button onClick={analyze} disabled={loading||sel.length<2} style={{width:"100%",padding:"8px",fontSize:"9px",letterSpacing:"1px",background:sel.length>=2&&!loading?"rgba(60,110,45,.14)":"rgba(255,255,255,.016)",border:`1px solid ${sel.length>=2&&!loading?"rgba(60,110,45,.28)":"rgba(255,255,255,.04)"}`,color:sel.length>=2&&!loading?"#7aaa62":"#1a1a0e",borderRadius:"4px",cursor:sel.length>=2?"pointer":"not-allowed"}}>
      ...
      {res&&<div style={{marginTop:"10px",padding:"12px",background:"rgba(255,255,255,.016)",border:"1px solid rgba(201,168,76,.1)",borderRadius:"4px"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:"8px",alignItems:"flex-start"}}>
          <div><div style={{fontSize:"12px",fontWeight:"bold",color:"#e0d0a0"}}>{name}</div>{res.era&&<div style={{fontSize:"8px",color:"#3a2a18",marginTop:"1px"}}>Typisch: {res.era}</div>}</div>
          <div style={{textAlign:"center"}}><div style={{fontSize:"19px",fontWeight:"bold",color:rCol(res.overallRating)}}>{res.overallRating}</div><div style={{fontSize:"11px",color:"#c9a84c"}}>{"⭐".repeat(res.stars||3)}</div></div>
        </div>
        {res.flavorText&&<div style={{fontSize:"9px",color:"#4a3a30",fontStyle:"italic",lineHeight:1.8,marginBottom:"8px",borderLeft:"3px solid rgba(201,168,76,.2)",paddingLeft:"8px"}}>{res.flavorText}</div>}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",marginBottom:"8px"}}>
          <div><div style={{fontSize:"6px",color:"#3a6a22",letterSpacing:"2px",marginBottom:"3px"}}>STÄRKEN</div>{res.strengths?.map((s,i)=><div key={i} style={{fontSize:"8px",color:"#2a5018",padding:"1px 0"}}>✓ {s}</div>)}</div>
          <div><div style={{fontSize:"6px",color:"#6a2a18",letterSpacing:"2px",marginBottom:"3px"}}>SCHWÄCHEN</div>{res.weaknesses?.map((w,i)=><div key={i} style={{fontSize:"8px",color:"#4a1810",padding:"1px 0"}}>✗ {w}</div>)}</div>
        </div>
        {res.worstVulnerability&&<div style={{padding:"6px 9px",background:"rgba(110,18,6,.09)",border:"1px solid rgba(110,18,6,.16)",borderRadius:"3px",marginBottom:"6px",fontSize:"8px",color:"#5a2a18"}}><strong style={{color:"#bb4422",display:"block",marginBottom:"1px"}}>Kritischste Schwachstelle:</strong>{res.worstVulnerability}</div>}
        {res.bestAttack&&<div style={{padding:"6px 9px",background:"rgba(100,15,6,.08)",border:"1px solid rgba(100,15,6,.14)",borderRadius:"3px",marginBottom:"6px",fontSize:"8px",color:"#5a2818",lineHeight:1.7}}><strong style={{color:"#aa3322",display:"block",marginBottom:"1px"}}>Angreifbar durch:</strong>{res.bestAttack}</div>}
        {res.historicalComp&&<div style={{fontSize:"8px",color:"#2a1a0a",fontStyle:"italic",borderTop:"1px solid rgba(255,255,255,.03)",paddingTop:"5px"}}>📜 {res.historicalComp}</div>}
      </div>}
    </div>
  );
}
...
function Compare({castles,init}){
  ...
  const ss={padding:"5px 9px",background:"rgba(0,0,0,.3)",border:"1px solid rgba(201,168,76,.12)",color:"#b09060",borderRadius:"3px",fontSize:"9px",width:"100%",outline:"none",fontFamily:"inherit"};
  return(
    <div>
      <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:"8px",marginBottom:"12px",alignItems:"center"}}>
        ...
        <div style={{color:"#1a1008",fontSize:"10px"}}>vs</div>
      ...
      {c1&&c2&&(<div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 36px 1fr",gap:"5px",textAlign:"center",marginBottom:"10px"}}>
          <div><div style={{fontSize:"17px"}}>{c1.icon}</div><div style={{fontSize:"9px",fontWeight:"bold",color:"#c0b090"}}>{c1.name}</div><div style={{fontSize:"18px",fontWeight:"bold",color:rCol(avg(c1))}}>{avg(c1)}</div></div>
          <div/>
          <div><div style={{fontSize:"17px"}}>{c2.icon}</div><div style={{fontSize:"9px",fontWeight:"bold",color:"#c0b090"}}>{c2.name}</div><div style={{fontSize:"18px",fontWeight:"bold",color:rCol(avg(c2))}}>{avg(c2)}</div></div>
        </div>
        {cats.map(cat=>{const v1=c1.ratings[cat.k],v2=c2.ratings[cat.k],w=v1>v2?"l":v2>v1?"r":"t";return(
          <div key={cat.k} style={{marginBottom:"6px"}}>
            <div style={{fontSize:"7px",color:"#1a0e08",letterSpacing:"2px",textAlign:"center",marginBottom:"2px"}}>{cat.l.toUpperCase()}</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 18px 1fr",gap:"3px",alignItems:"center"}}>
              <div style={{display:"flex",justifyContent:"flex-end",gap:"3px",alignItems:"center"}}><span style={{fontSize:"8px",fontWeight:"bold",color:rCol(v1)}}>{v1}</span><div style={{height:"4px",width:`${v1*.55}px`,maxWidth:"55px",background:rCol(v1),borderRadius:"2px 0 0 2px",opacity:w==="l"?1:.25}}/></div>
              <div style={{textAlign:"center",fontSize:"8px",color:w==="t"?"#c9a84c":"#1a0e08"}}>{w==="l"?"◀":w==="r"?"▶":"="}</div>
              <div style={{display:"flex",gap:"3px",alignItems:"center"}}><div style={{height:"4px",width:`${v2*.55}px`,maxWidth:"55px",background:rCol(v2),borderRadius:"0 2px 2px 0",opacity:w==="r"?1:.25}}/><span style={{fontSize:"8px",fontWeight:"bold",color:rCol(v2)}}>{v2}</span></div>
            </div>
          </div>);})}
        ...
        <div style={{marginTop:"8px",padding:"8px",textAlign:"center",background:"rgba(201,168,76,.03)",border:"1px solid rgba(201,168,76,.08)",borderRadius:"4px",fontSize:"9px",color:"#5a4a30"}}>
          ...
          return(
            <button key={c.id} onClick={()=>onSelect(c)} title={`${c.name} · ${c.era}`} style={{position:"absolute",left:`${pct}%`,top:isUp?"4px":"68px",transform:"translateX(-50%)",background:"transparent",border:"none",cursor:"pointer",padding:0}}>
              <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"1px"}}>
                {isUp&&<div style={{fontSize:"7px",color:"#3a2a14",whiteSpace:"nowrap",maxWidth:"60px",overflow:"hidden",textOverflow:"ellipsis",textAlign:"center"}}>{c.name}</div>}
                ...
                {!isUp&&<div style={{fontSize:"7px",color:"#3a2a14",whiteSpace:"nowrap",maxWidth:"60px",overflow:"hidden",textOverflow:"ellipsis",textAlign:"center"}}>{c.name}</div>}
                <div style={{fontSize:"6px",color:"#1a0e08",fontFamily:"monospace"}}>{c.year>0?c.year:`${Math.abs(c.year)}v`}</div>
              </div>
            </button>
          );
        })}
      </div>
      <div style={{display:"flex",justifyContent:"space-between",fontSize:"7px",color:"#1a0e08",fontFamily:"monospace",marginBottom:"18px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:"5px"}}>
        {sorted.map(c=>(
          <button key={c.id} onClick={()=>onSelect(c)} style={{padding:"8px 10px",textAlign:"left",background:"rgba(255,255,255,.016)",border:"1px solid rgba(255,255,255,.035)",borderRadius:"4px",cursor:"pointer",display:"flex",gap:"8px",alignItems:"center",transition:"all .12s"}}>
            ...
            <div style={{flex:1,minWidth:0}}><div style={{fontSize:"9px",fontWeight:"bold",color:"#9a8860",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.name}</div><div style={{fontSize:"7px",color:"#1a0e08"}}>{c.era}</div></div>
            <div style={{fontSize:"10px",fontWeight:"bold",color:rCol(avg(c)),flexShrink:0}}>{avg(c)}</div>
          </button>
        ))}
      </div>
    </div>
  ...
  if(phase==="results")return(
    <div>
      <div style={{textAlign:"center",padding:"14px",marginBottom:"14px",background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.16)",borderRadius:"6px"}}>
        ...
        <div style={{fontSize:"9px",color:"#7a6840"}}>{wins}/{results.length} Burgen eingenommen · Gesamt: {total}/{results.length*10}</div>
      </div>
      {results.map((r,i)=>(
        <div key={i} style={{padding:"10px 12px",background:r.success?"rgba(22,70,12,.1)":"rgba(80,12,7,.1)",border:`1px solid ${r.success?"rgba(35,95,18,.2)":"rgba(100,15,8,.2)"}`,borderRadius:"4px",marginBottom:"5px",display:"flex",gap:"9px",alignItems:"flex-start"}}>
          ...
          <div style={{flex:1}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:"2px"}}><span style={{fontSize:"9px",fontWeight:"bold",color:"#c0b090"}}>{r.castle.name}</span>
              <div style={{display:"flex",gap:"7px",alignItems:"center"}}><span style={{fontSize:"7px",color:r.success?"#5a9a42":"#aa3322"}}>{r.success?"✅":"❌"}</span><span style={{fontSize:"12px",fontWeight:"bold",color:rCol(r.rating*10)}}>{r.rating}/10</span></div></div>
            <div style={{fontSize:"8px",color:"#4a3a22",lineHeight:1.6}}>{r.outcome}</div>
            {r.days&&<div style={{fontSize:"7px",color:"#2a1a0a",marginTop:"1px"}}>~{r.days} Tage</div>}
          </div>
        </div>
      ))}
      <button onClick={()=>{setPhase("pick");setSel([]);setResults([]);setCur(0);}} style={{width:"100%",marginTop:"10px",padding:"8px",background:"rgba(201,168,76,.08)",border:"1px solid rgba(201,168,76,.2)",color:"#c9a84c",borderRadius:"4px",cursor:"pointer",fontSize:"9px",letterSpacing:"1px"}}>NEUES TURNIER</button>
    </div>
  );
  if(phase==="siege"){
    const castle=sel[cur];
    return(<div>
      <div style={{display:"flex",gap:"4px",marginBottom:"10px",flexWrap:"wrap"}}>
        {sel.map((c,i)=><div key={i} style={{padding:"4px 8px",background:i===cur?`${c.theme.glow}`:"rgba(255,255,255,.02)",border:`1px solid ${i===cur?c.theme.accent+"44":"rgba(255,255,255,.04)"}`,borderRadius:"3px",fontSize:"9px",color:i===cur?c.theme.accent:i<cur?"#5a9a42":"#1a0e08",display:"flex",gap:"3px",alignItems:"center"}}>{c.icon}{i<cur?"✅":i===cur?"⚔️":""}</div>)}
      </div>
      <div style={{padding:"9px 11px",background:`${castle.theme.glow}`,border:`1px solid ${castle.theme.accent}33`,borderRadius:"4px",marginBottom:"10px",display:"flex",gap:"8px",alignItems:"center"}}>
        ...
        <div><div style={{fontSize:"10px",fontWeight:"bold",color:"#e0d0b0"}}>{castle.name}</div><div style={{fontSize:"8px",color:"#4a3a22"}}>{castle.sub} · {avg(castle)}</div></div>
        <div style={{marginLeft:"auto",fontSize:"8px",color:"#2a1a0a"}}>Burg {cur+1}/{sel.length}</div>
      </div>
      <div style={{fontSize:"7px",color:"#2a1a0a",letterSpacing:"2px",marginBottom:"7px",display:"flex",justifyContent:"space-between"}}><span>STRATEGIE</span><span style={{color:used===TOTAL_RES?"#8aaa68":"#c9a84c"}}>{used}/{TOTAL_RES}</span></div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px",marginBottom:"9px"}}>
        {Object.entries(RES).slice(0,10).map(([k,r])=>(
          <div key={k} style={{padding:"6px 8px",background:"rgba(255,255,255,.014)",border:`1px solid ${alloc[k]>0?`${r.c}24`:"rgba(255,255,255,.028)"}`,borderRadius:"3px"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:"3px"}}><div style={{fontSize:"7px",color:alloc[k]>0?r.c:"#1a0e0e"}}>{r.i} {r.l}</div><div style={{fontSize:"10px",fontWeight:"bold",color:alloc[k]>0?r.c:"#0a0600"}}>{alloc[k]}</div></div>
      ...
      <button onClick={run} disabled={loading||used<2} style={{width:"100%",padding:"8px",fontSize:"9px",letterSpacing:"1px",background:used>=2&&!loading?"rgba(140,55,10,.16)":"rgba(255,255,255,.016)",border:`1px solid ${used>=2&&!loading?"rgba(140,55,10,.34)":"rgba(255,255,255,.04)"}`,color:used>=2&&!loading?"#cc6633":"#0e0600",borderRadius:"4px",cursor:used>=2?"pointer":"not-allowed"}}>
        {loading?`⏳ Stürme ${castle.name}…`:`⚔️ ${castle.name} ANGREIFEN`}
      </button>
    </div>);
  }
  return(<div>
    <div style={{padding:"9px 12px",background:"rgba(201,168,76,.05)",border:"1px solid rgba(201,168,76,.14)",borderRadius:"4px",marginBottom:"12px",fontSize:"10px",color:"#7a6a40",lineHeight:1.7}}>
    ...
    <div style={{fontSize:"7px",color:"#2a1a0a",letterSpacing:"1px",marginBottom:"6px"}}>{sel.length}/{MAX} AUSGEWÄHLT</div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"4px",marginBottom:"10px",maxHeight:"300px",overflowY:"auto"}}>
      {castles.map(c=>{const iS=sel.find(x=>x.id===c.id),canA=sel.length<MAX||iS;return(
        <button key={c.id} onClick={()=>toggle(c)} style={{padding:"7px 5px",textAlign:"center",background:iS?`${c.theme.glow}`:"rgba(255,255,255,.016)",border:`1px solid ${iS?c.theme.accent+"44":"rgba(255,255,255,.035)"}`,borderRadius:"4px",cursor:canA?"pointer":"not-allowed",opacity:canA?1:.28,transition:"all .11s"}}>
          ...
          <div style={{fontSize:"7px",fontWeight:"bold",color:iS?c.theme.accent:"#5a4a30",lineHeight:1.2}}>{c.name.split(" ").slice(0,2).join(" ")}</div>
          <div style={{fontSize:"7px",color:rCol(avg(c)),marginTop:"1px"}}>{avg(c)}</div>
          {iS&&<div style={{fontSize:"7px",color:c.theme.accent,marginTop:"1px"}}>✓</div>}
        </button>);})}
    </div>
    <button onClick={()=>{if(sel.length>=2){setPhase("siege");setCur(0);setResults([]);}}} disabled={sel.length<2} style={{width:"100%",padding:"8px",fontSize:"9px",letterSpacing:"1px",background:sel.length>=2?"rgba(201,168,76,.09)":"rgba(255,255,255,.016)",border:`1px solid ${sel.length>=2?"rgba(201,168,76,.25)":"rgba(255,255,255,.035)"}`,color:sel.length>=2?"#c9a84c":"#1a1008",borderRadius:"4px",cursor:sel.length>=2?"pointer":"not-allowed"}}>
...
function Highscores({scores,onSelect}){
  ...
  if(!total) return(
    <div style={{padding:"40px",textAlign:"center"}}>
      ...
      <div style={{fontSize:"12px",color:"#3a2a14",marginBottom:"6px"}}>Noch keine Belagerungen.</div>
      <div style={{fontSize:"9px",color:"#2a1a0a"}}>Starte den Simulator oder das Rollenspiel um Scores zu sammeln.</div>
    </div>
  );
  ...
  return(
    <div style={{padding:"18px 20px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"8px",marginBottom:"18px"}}>
        {[
          ...
          <div key={s.l} style={{padding:"10px 8px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.04)",borderRadius:"4px",textAlign:"center"}}>
            ...
            <div style={{fontSize:"7px",color:"#2a1a0a",letterSpacing:"1px",marginTop:"2px"}}>{s.l.toUpperCase()}</div>
          </div>
        ))}
      </div>
      {/* Win rate bar */}
      <div style={{marginBottom:"16px",padding:"10px 12px",background:"rgba(0,0,0,0.2)",border:"1px solid rgba(255,255,255,0.04)",borderRadius:"4px"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:"5px",fontSize:"8px",color:"#3a2a14"}}>
        ...
        {hardest&&<div style={{marginTop:"8px",fontSize:"8px",color:"#3a2a14"}}>Härtester Gegner: <span style={{color:"#cc5544"}}>{hardest.icon} {hardest.name}</span></div>}
        {easiest&&<div style={{marginTop:"2px",fontSize:"8px",color:"#3a2a14"}}>Leichtester Sieg: <span style={{color:"#8aaa68"}}>{easiest.icon} {easiest.name}</span></div>}
      </div>
      {/* Entries */}
      <div style={{display:"flex",flexDirection:"column",gap:"5px"}}>
        {entries.map((e,i)=>(
          <div key={e.id}
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:"10px",fontWeight:"bold",color:"#b09870",marginBottom:"1px"}}>{e.name}</div>
              <div style={{fontSize:"8px",color:e.won?"#4a7a32":"#7a2a18",display:"flex",gap:"8px"}}>
            ...
            <div style={{textAlign:"center",flexShrink:0}}>
              ...
              <div style={{fontSize:"6px",color:"#1a0e08"}}>/10</div>
            </div>
          </div>
        ))}
      </div>
...
function Lexikon({castle,onAsk}){
  const facts=LEXIKON_FACTS[castle.id]||[];
  const [loading,setLoading]=useState(false);
  ...
  const getFact=async()=>{
    ...
    try{
      const r=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:400,messages:[{role:"user",content:`Nenne einen außergewöhnlichen, wenig bekannten historischen Fakt über "${castle.name}" (${castle.era}, ${castle.loc}). Der Fakt soll überraschend, präzise und fesselnd sein. Antwort auf Deutsch, 2-3 Sätze, kein Markdown.`}]})});
      const d=await r.json();
      setExtraFact(d.content?.map(b=>b.text||"").join("")||"");
    }catch{setExtraFact("Verbindungsfehler.");}
    setLoading(false);
  };
  ...
  const askQuestion=async()=>{
    ...
    try{
      const r=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:500,messages:[{role:"user",content:`Historiker über "${castle.name}" (${castle.era}, ${castle.loc}). Kontext: ${castle.history}. Frage: ${q}. Antworte präzise auf Deutsch, 2-4 Sätze.`}]})});
      const d=await r.json();
      setExtraFact(d.content?.map(b=>b.text||"").join("")||"");
    }catch{setExtraFact("Verbindungsfehler.");}
    setLoading(false);
    ...
    <div style={{maxWidth:"600px"}}>
      ...
      <div style={{padding:"12px 14px",background:`${castle.theme.accent}08`,border:`1px solid ${castle.theme.accent}18`,borderRadius:"4px",marginBottom:"14px",display:"flex",gap:"12px",alignItems:"center"}}>
        ...
        <div>
          ...
          <div style={{fontSize:"9px",color:castle.theme.accent,marginTop:"1px"}}>{castle.sub} · {castle.era} · {castle.loc}</div>
        </div>
      </div>
      ...
      {facts.length>0&&(
        <div style={{marginBottom:"14px"}}>
          <div style={{fontSize:"7px",color:castle.theme.accent,letterSpacing:"2px",marginBottom:"8px"}}>📚 BEKANNTE FAKTEN</div>
          {facts.map((f,i)=>(
            <div key={i} style={{display:"flex",gap:"9px",padding:"8px 10px",marginBottom:"4px",
              ...
              <div style={{fontSize:"9px",color:castle.theme.accent,flexShrink:0,marginTop:"1px",fontFamily:"monospace"}}>{String(i+1).padStart(2,"0")}</div>
              <div style={{fontSize:"10px",color:"#5a4a30",lineHeight:1.75}}>{f}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {extraFact&&(
        <div style={{marginBottom:"12px",padding:"12px 14px",
          ...
          <div style={{fontSize:"7px",color:castle.theme.accent,letterSpacing:"2px",marginBottom:"5px"}}>✨ ÜBERRASCHUNGSFAKT</div>
          <div style={{fontSize:"10px",color:"#7a6a48",lineHeight:1.8}}>{extraFact}</div>
        </div>
      )}
      {loading&&(
        <div style={{padding:"12px",textAlign:"center",color:"#3a2a14",fontSize:"10px",marginBottom:"12px"}}>
      ...
      <div style={{display:"flex",gap:"6px",marginBottom:"12px"}}>
        <button onClick={getFact} disabled={loading}
          style={{flex:1,padding:"8px",fontSize:"9px",letterSpacing:"1px",
      ...
      <div style={{padding:"12px",background:"rgba(0,0,0,0.2)",border:"1px solid rgba(255,255,255,0.04)",borderRadius:"4px"}}>
        <div style={{fontSize:"7px",color:"#3a2a14",letterSpacing:"2px",marginBottom:"7px"}}>❓ FRAGE STELLEN</div>
        <div style={{display:"flex",gap:"5px"}}>
          <input value={question} onChange={e=>setQuestion(e.target.value)}
            ...
            style={{flex:1,padding:"7px 10px",background:"rgba(255,255,255,0.04)",
              ...
              color:"#e8d8b0",fontSize:"10px",outline:"none",fontFamily:"inherit"}}/>
        ...
        <div style={{display:"flex",gap:"4px",flexWrap:"wrap",marginTop:"6px"}}>
          {["Wann genau fiel sie?","Wer baute sie?","Was ist heute davon übrig?","Vergleich mit anderen Burgen?"].map((q,i)=>(
            <button key={i} onClick={()=>setQuestion(q)}
              style={{fontSize:"8px",padding:"2px 7px",background:"rgba(255,255,255,0.02)",
                border:"1px solid rgba(255,255,255,0.05)",color:"#2a1a0a",borderRadius:"8px",cursor:"pointer"}}>
              {q}
            </button>
          ))}
        </div>
    ...
    <div style={{minHeight:"100vh",background:"#060504",color:"#e8dcc8",fontFamily:"'Palatino Linotype','Book Antiqua',Georgia,serif",display:"flex",flexDirection:"column"}}>
      ...
      <header style={{height:"46px",display:"flex",alignItems:"stretch",borderBottom:"1px solid rgba(201,168,76,0.07)",background:"rgba(5,4,3,0.99)",position:"sticky",top:0,zIndex:300,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:"7px",padding:"0 12px",borderRight:"1px solid rgba(255,255,255,0.04)",flexShrink:0}}>
          ...
          <span style={{fontSize:"10px",fontWeight:"bold",color:"#f0e6cc",letterSpacing:"2px",whiteSpace:"nowrap"}}>BELAGERUNGS-ATLAS</span>
          <span style={{fontSize:"8px",color:"#3a2a10",borderLeft:"1px solid rgba(255,255,255,0.05)",paddingLeft:"7px"}}>{CASTLES.length} Burgen</span>
        </div>
        <div style={{display:"flex",height:"100%",flex:1,overflowX:"auto"}}>
          {NAVTABS.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{height:"100%",padding:"0 12px",background:"transparent",border:"none",borderBottom:`2px solid ${tab===t.id?"#c9a84c":"transparent"}`,color:tab===t.id?"#c9a84c":"#2a1a08",cursor:"pointer",fontSize:"9px",letterSpacing:"0.5px",transition:"all .14s",marginBottom:"-1px",whiteSpace:"nowrap"}}>{t.l}</button>
          ))}
        </div>
        {/* Setup button */}
        <button onClick={()=>setShowSetup(s=>!s)} style={{padding:"0 12px",background:showSetup?"rgba(201,168,76,0.08)":"transparent",border:"none",borderBottom:`2px solid ${showSetup?"#c9a84c":"transparent"}`,borderLeft:"1px solid rgba(255,255,255,0.04)",color:showSetup?"#c9a84c":"#2a1a08",cursor:"pointer",fontSize:"9px",whiteSpace:"nowrap",marginBottom:"-1px"}}>
      ...
      {showSetup&&(
        <div style={{background:"rgba(8,6,4,0.98)",borderBottom:"1px solid rgba(201,168,76,0.1)",padding:"12px 14px",display:"flex",gap:"16px",flexWrap:"wrap"}}>
          <div>
            <div style={{fontSize:"7px",color:"#3a2a14",letterSpacing:"2px",marginBottom:"5px"}}>⚔️ GENERAL</div>
            <div style={{display:"flex",gap:"4px",flexWrap:"wrap"}}>
              <button onClick={()=>setGeneral(null)} style={{padding:"3px 8px",fontSize:"8px",background:!general?"rgba(201,168,76,0.08)":"rgba(255,255,255,0.02)",border:`1px solid ${!general?"rgba(201,168,76,0.25)":"rgba(255,255,255,0.05)"}`,color:!general?"#c9a84c":"#2a1a0a",borderRadius:"2px",cursor:"pointer"}}>Keiner</button>
              {GENERALS.map(g=>(
                <button key={g.id} onClick={()=>setGeneral(g===general?null:g)} title={g.bio} style={{padding:"3px 8px",fontSize:"8px",background:general?.id===g.id?"rgba(201,168,76,0.08)":"rgba(255,255,255,0.02)",border:`1px solid ${general?.id===g.id?"rgba(201,168,76,0.25)":"rgba(255,255,255,0.05)"}`,color:general?.id===g.id?"#c9a84c":"#2a1a0a",borderRadius:"2px",cursor:"pointer"}}>
                  {g.emoji} {g.name}
                </button>
              ))}
            </div>
          ...
          <div>
            <div style={{fontSize:"7px",color:"#3a2a14",letterSpacing:"2px",marginBottom:"5px"}}>🌿 JAHRESZEIT</div>
            <div style={{display:"flex",gap:"4px"}}>
              {SEASONS.map(s=>(
                <button key={s.id} onClick={()=>setSeason(s)} title={s.desc} style={{padding:"3px 8px",fontSize:"8px",background:season?.id===s.id?"rgba(201,168,76,0.08)":"rgba(255,255,255,0.02)",border:`1px solid ${season?.id===s.id?"rgba(201,168,76,0.25)":"rgba(255,255,255,0.05)"}`,color:season?.id===s.id?"#c9a84c":"#2a1a0a",borderRadius:"2px",cursor:"pointer"}}>
                  {s.emoji} {s.name}
                </button>
              ))}
            </div>
          </div>
          {general&&<div style={{padding:"6px 10px",background:"rgba(0,0,0,0.3)",border:"1px solid rgba(255,255,255,0.04)",borderRadius:"4px",fontSize:"9px",color:"#5a4a28"}}>
            <strong style={{color:general?"#c9a84c":"#3a2a14"}}>{general.emoji} {general.name}</strong> — {general.specialty}<br/>
            <span style={{fontSize:"8px",color:"#3a2a14"}}>{Object.entries(general.bonus).map(([k,v])=>`${k} +${v}`).join(" · ")}</span>
          </div>}
        </div>
      )}
      ...
      {tab==="build"&&<div style={{flex:1,overflowY:"auto",padding:"18px",maxWidth:"680px",margin:"0 auto",width:"100%"}}>
        ...
        <p style={{fontSize:"9px",color:"#3a2a18",margin:"0 0 14px",lineHeight:1.7}}>Budget: {BUILDER_BUDGET} Gold · {BUILDER.length} Elemente · Einschließlich Drachen & Magie</p>
        <CastleBuilder/>
      </div>}
      ...
      {tab==="detail"&&(
        <div style={{display:"flex",flex:1,overflow:"hidden"}}>
          ...
          <aside style={{width:sideOpen?"195px":"40px",flexShrink:0,transition:"width .2s ease",borderRight:"1px solid rgba(201,168,76,0.05)",background:"rgba(0,0,0,0.16)",overflowY:"auto",overflowX:"hidden"}}>
            <button onClick={()=>setSideOpen(e=>!e)} style={{width:"100%",padding:"6px",background:"transparent",border:"none",borderBottom:"1px solid rgba(255,255,255,0.02)",color:"#1a0e04",cursor:"pointer",fontSize:"10px",textAlign:sideOpen?"right":"center"}}>{sideOpen?"◀":"▶"}</button>
            {sideFiltered.map(c=>{const a=avg(c),isA=sel.id===c.id,hs=scores[c.id];return(
              <button key={c.id} onClick={()=>{setSel(c);setDtab("map");setAttackMode(false);}} title={c.name}
                ...
                {sideOpen&&<><div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:"8px",fontWeight:"bold",color:isA?"#f0e6cc":"#5a4a30",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.name}</div>
                  <div style={{fontSize:"6px",color:c.type==="real"?"#1a4010":"#1a1a48"}}>{c.type==="real"?"⚜":"✦"} {c.epoch}</div>
                </div>
                <div style={{flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"1px"}}>
                  <span style={{fontSize:"8px",fontWeight:"bold",color:rCol(a)}}>{a}</span>
                  {hs&&<span style={{fontSize:"6px",color:hs.won?"#4a7a32":"#7a2a18"}}>{hs.won?"✅":"❌"}</span>}
                </div></>}
              </button>
            );})}
          </aside>
              ...
              <div style={{display:"flex",gap:"10px",alignItems:"flex-start"}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",gap:"4px",alignItems:"center",marginBottom:"2px",flexWrap:"wrap"}}>
                    <span style={{fontSize:"7px",letterSpacing:"2px",padding:"1px 6px",borderRadius:"2px",background:sel.type==="real"?"rgba(40,70,25,0.18)":"rgba(40,40,90,0.18)",color:sel.type==="real"?"#5a7a38":"#6a6aaa",border:`1px solid ${sel.type==="real"?"rgba(40,70,25,0.28)":"rgba(40,40,90,0.28)"}`}}>{sel.type==="real"?"⚜ HISTORISCH":"✦ FANTASY"}</span>
                    <span style={{fontSize:"7px",color:"#1a0e08"}}>{sel.epoch} · {sel.loc} · {sel.era}</span>
                  </div>
                  <h1 style={{fontSize:"15px",fontWeight:"bold",color:"#f5edd8",margin:"0 0 1px"}}>{sel.name}</h1>
                  <div style={{fontSize:"8px",color:sel.theme.accent,marginBottom:"3px"}}>{sel.sub}</div>
                  <p style={{fontSize:"9px",color:"#3a2a22",lineHeight:1.7,margin:0,maxWidth:"480px"}}>{sel.desc}</p>
                </div>
                <div style={{flexShrink:0,textAlign:"center",padding:"8px 11px",background:"rgba(0,0,0,0.35)",border:`1px solid ${sel.theme.accent}1e`,borderRadius:"4px"}}>
                  ...
                  <div style={{fontSize:"6px",color:"#100800",letterSpacing:"2px",marginTop:"1px"}}>GESAMT</div>
                  {general&&<div style={{fontSize:"8px",color:sel.theme.accent,marginTop:"2px"}}>{general.emoji}</div>}
                  {season&&<div style={{fontSize:"9px",marginTop:"1px"}}>{season.emoji}</div>}
                </div>
              </div>
            </div>
              ...
              {dtab==="map"&&(
                <div>
                  <div style={{marginBottom:"8px",display:"flex",gap:"6px",alignItems:"center"}}>
                    <div style={{fontSize:"7px",color:sel.theme.accent,letterSpacing:"2px"}}>🗺 GRUNDRISS — KLICKE BEREICHE AN</div>
                    <div style={{flex:1}}/>
                    <div style={{fontSize:"8px",color:"#3a2a14"}}>{sel.epoch} · {sel.loc}</div>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"minmax(260px,340px) 1fr",gap:"14px"}}>
                    ...
                    <div>
                      <div style={{fontSize:"7px",color:sel.theme.accent,letterSpacing:"2px",marginBottom:"6px"}}>ALLE BEREICHE</div>
                      {sel.zones.map(z=>(
                        <div key={z.id} style={{display:"flex",gap:"6px",padding:"7px 9px",marginBottom:"4px",background:"rgba(255,255,255,.013)",border:"1px solid rgba(255,255,255,.025)",borderLeft:`3px solid ${z.c}`,borderRadius:"3px"}}>
                          <div style={{flex:1}}>
                            <div style={{fontSize:"8px",fontWeight:"bold",color:"#907850",marginBottom:"2px"}}>{z.l}</div>
                            <div style={{fontSize:"7px",color:"#1a0e10",lineHeight:1.6}}>{z.d}</div>
                      ...
                      <div style={{marginTop:"10px",padding:"9px 11px",background:"rgba(255,255,255,0.015)",border:`1px solid ${sel.theme.accent}12`,borderRadius:"4px"}}>
                        <div style={{fontSize:"7px",color:sel.theme.accent,letterSpacing:"2px",marginBottom:"6px"}}>SCHNELLWERTUNG</div>
                        {[["Mauern",sel.ratings.walls],["Position",sel.ratings.position],["Versorgung",sel.ratings.supply]].map(([l,v])=>(
                          <div key={l} style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"4px"}}>
                            <div style={{fontSize:"7px",color:"#2a1a08",width:"52px"}}>{l}</div>
                            ...
                            <div style={{fontSize:"8px",color:rCol(v),fontFamily:"monospace",width:"22px",textAlign:"right"}}>{v}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
                  <div>
                    <div style={{padding:"12px",background:"rgba(255,255,255,.016)",border:`1px solid ${sel.theme.accent}16`,borderRadius:"4px",marginBottom:"8px"}}>
                      <div style={{fontSize:"7px",color:sel.theme.accent,letterSpacing:"2px",marginBottom:"9px"}}>DEFENSIV-RATING</div>
                      ...
                      <div style={{marginTop:"9px",paddingTop:"8px",borderTop:"1px solid rgba(255,255,255,.03)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <span style={{fontSize:"7px",color:"#1a0e0a",letterSpacing:"2px"}}>GESAMT</span>
                    ...
                    <div style={{padding:"10px",background:"rgba(255,255,255,.013)",border:`1px solid ${sel.theme.accent}12`,borderRadius:"4px"}}>
                      <div style={{fontSize:"7px",color:sel.theme.accent,letterSpacing:"2px",marginBottom:"6px"}}>RADAR-PROFIL</div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:"5px"}}>
                    {[{t:"STÄRKEN",items:sel.strengths,c:"rgba(40,70,22,.07)",bc:"rgba(40,70,22,.13)",tc:"#3a5a1e",ic:"#4a6a28"},{t:"SCHWÄCHEN",items:sel.weaknesses,c:"rgba(80,20,10,.07)",bc:"rgba(80,20,10,.13)",tc:"#4a2010",ic:"#622a18"},{t:"ANGRIFFSTIPPS",items:sel.attackTips,c:"rgba(100,40,6,.05)",bc:"rgba(100,40,6,.1)",tc:"#5a3018",ic:"#c9a84c"}].map(g=>(
                      <div key={g.t} style={{padding:"9px 10px",background:g.c,border:`1px solid ${g.bc}`,borderRadius:"4px"}}>
                        <div style={{fontSize:"6px",color:g.ic,letterSpacing:"2px",marginBottom:"5px"}}>{g.t}</div>
                        {g.items.map((s,i)=><div key={i} style={{fontSize:"8px",color:g.tc,padding:"2px 0",borderBottom:i<g.items.length-1?"1px solid rgba(255,255,255,.018)":"none",display:"flex",gap:"4px"}}><span style={{color:g.ic,flexShrink:0}}>›</span>{s}</div>)}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {dtab==="history"&&(
                <div style={{maxWidth:"620px"}}>
                  ...
                  <div style={{marginBottom:"16px"}}>
                    <div style={{fontSize:"7px",color:sel.theme.accent,letterSpacing:"2px",marginBottom:"10px"}}>⏳ HISTORISCHER ZEITSTRAHL</div>
                      ...
                      {[
                        ...
                        <div key={i} style={{position:"relative",marginBottom:"14px",paddingLeft:"14px"}}>
                          ...
                          <div style={{
                            ...
                            <div style={{fontSize:"7px",color:e.color,letterSpacing:"1.5px",marginBottom:"4px",fontWeight:"bold"}}>{e.label.toUpperCase()}</div>
                            <div style={{fontSize:"10px",color:e.highlight?"#c0b090":"#4a3a28",lineHeight:1.8}}>{e.text}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",marginBottom:"10px"}}>
                    <div style={{padding:"10px 12px",background:"rgba(40,70,22,0.07)",border:"1px solid rgba(40,70,22,0.14)",borderRadius:"4px"}}>
                      <div style={{fontSize:"7px",color:"#5a8a38",letterSpacing:"2px",marginBottom:"6px"}}>⚡ STÄRKEN</div>
                      {sel.strengths.map((s,i)=>(
                        <div key={i} style={{fontSize:"9px",color:"#3a5820",padding:"2px 0",borderBottom:i<sel.strengths.length-1?"1px solid rgba(255,255,255,0.02)":"none",display:"flex",gap:"5px"}}>
                    ...
                    <div style={{padding:"10px 12px",background:"rgba(80,20,10,0.07)",border:"1px solid rgba(80,20,10,0.14)",borderRadius:"4px"}}>
                      <div style={{fontSize:"7px",color:"#8a3a20",letterSpacing:"2px",marginBottom:"6px"}}>⚠ SCHWÄCHEN</div>
                      {sel.weaknesses.map((w,i)=>(
                        <div key={i} style={{fontSize:"9px",color:"#5a2818",padding:"2px 0",borderBottom:i<sel.weaknesses.length-1?"1px solid rgba(255,255,255,0.02)":"none",display:"flex",gap:"5px"}}>
                  ...
                  <div style={{padding:"11px 13px",background:`${sel.theme.accent}07`,border:`1px solid ${sel.theme.accent}18`,borderRadius:"4px"}}>
                    <div style={{fontSize:"7px",color:sel.theme.accent,letterSpacing:"2px",marginBottom:"5px"}}>⚔️ BELAGERUNGSSZENARIO</div>
                    <p style={{fontSize:"10px",color:"#5a4828",lineHeight:1.9,margin:0,fontStyle:"italic"}}>{sel.siegeCtx}</p>
                  </div>
                </div>
              )}
            </div>

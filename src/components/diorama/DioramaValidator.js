...
/**
 * Validation Loop (Principle 5): given a validation report and the current
 * component list, attempt to auto-correct unreachable floor levels by injecting
 * STAIR_FLIGHT components that connect each unreachable level to the nearest
 * reachable one.
 *
 * Returns a NEW component array (does not mutate the input).  The injected stairs
 * are marked with `autoGenerated: true` so they can be distinguished from
 * hand-authored components and later replaced with properly positioned ones.
 *
 * @param  {Array}            components  — current component list
 * @param  {ValidationReport} report      — from validateDiorama()
 * @returns {Array}  — component array, possibly with added STAIR_FLIGHT items
 */
export function autoCorrectReachability(components, report) {
  if (report.unreachableLevels.length === 0) return components;

  const additions = [];
  const reachable = [...report.reachableLevels];

  for (const targetY of report.unreachableLevels) {
    // Find nearest already-reachable level to connect from
    const fromY = reachable.reduce(
      (best, l) => Math.abs(l - targetY) < Math.abs(best - targetY) ? l : best,
      reachable[0] ?? 0,
    );
    const riseH = targetY - fromY;
    if (Math.abs(riseH) < 0.3) continue; // gap too small — probably a rounding artefact

    const steps   = Math.max(4, Math.round(Math.abs(riseH) / 0.22));
    const totalD  = steps * 0.35;

    additions.push({
      type:          'STAIR_FLIGHT',
      id:            `auto/stair_flight/${fromY.toFixed(1)}-${targetY.toFixed(1)}`,
      label:         `Auto-Treppe ${fromY.toFixed(1)}m → ${targetY.toFixed(1)}m`,
      autoGenerated: true,
      x:             0,
      z:             0,
      y:             fromY,
      steps,
      stepH:         Math.abs(riseH) / steps,
      stepD:         0.35,
      w:             2.0,
      rotation:      0,
    });

    // Mark target level as now reachable so subsequent unreachable levels
    // can chain off it if needed.
    reachable.push(targetY);
  }

  if (additions.length === 0) return components;

  console.info(
    `[DioramaValidator] autoCorrect: injected ${additions.length} STAIR_FLIGHT(s) for unreachable levels:`,
    report.unreachableLevels,
  );
